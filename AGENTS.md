# SmartTutor — Agent-Native Architecture

## Overview

SmartTutor is an **agent-native** intelligent learning companion organized
...
`smarttutor/capabilities/_shared.py` so every turn emits the same envelope
...
```bash
# Install
pip install smarttutor      # Full app (CLI + Web/API + packaged Web assets)
pip install smarttutor-cli  # CLI-only

# Run any capability
smarttutor run chat "Explain Fourier transform"
smarttutor run deep_solve "Solve x^2=4" -t rag --kb my-kb
smarttutor run visualize "Animate sine wave" --config render_mode=manim_video
...
smarttutor partner list

# Knowledge bases, memory, server
smarttutor kb list
smarttutor kb create my-kb --doc textbook.pdf
smarttutor memory show
smarttutor serve --port 8001       # API server only
smarttutor start                   # backend + frontend together
```
...
| Path                                       | Purpose                              |
| ------------------------------------------ | ------------------------------------ |
| `smarttutor/runtime/orchestrator.py`        | `ChatOrchestrator` — unified entry   |
| `smarttutor/runtime/launcher.py`            | Backend + frontend lifecycle / port discovery |
| `smarttutor/runtime/registry/`              | Tool + Capability registries         |
| `smarttutor/runtime/bootstrap/builtin_capabilities.py` | Built-in capability class paths |
| `smarttutor/services/config/runtime_settings.py` | JSON settings + process-env overrides |
| `smarttutor/core/stream.py`, `stream_bus.py` | StreamEvent protocol + async fan-out |
| `smarttutor/core/tool_protocol.py`          | `BaseTool` + `ToolDefinition`         |
| `smarttutor/core/capability_protocol.py`    | `BaseCapability` + `CapabilityManifest` |
| `smarttutor/core/context.py`                | `UnifiedContext` dataclass            |
| `smarttutor/tools/builtin/__init__.py`      | All built-in tool wrappers           |
| `smarttutor/capabilities/`                  | Built-in capability implementations  |
| `smarttutor/app.py`                         | `SmartTutorApp` — Python SDK facade    |
| `smarttutor_cli/main.py`                    | Typer CLI entry point                |
| `smarttutor/api/routers/unified_ws.py`      | Unified WebSocket endpoint           |
...
```
pip install smarttutor      — Full app (CLI + Web/API + packaged Web assets)
pip install smarttutor-cli  — CLI-only (LLM + RAG + providers + document parsing)
...
.[math-animator]  — Manim addon (powers `visualize` Manim renders + `smarttutor run math_animator`)
