# ============================================
# Smart Tutor Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for Smart Tutor
# containing both the FastAPI backend and Next.js frontend
#
# Build/run:
#   docker build -t smarttutor:local .
#   docker run -p 127.0.0.1:3782:3782 -p 127.0.0.1:8001:8001 \
#     -v smarttutor-data:/app/data smarttutor:local
#
# Prerequisites:
...
COPY smarttutor/__version__.py /app/smarttutor/__version__.py
...
# SMARTTUTOR_API_BASE_URL (exported by the entrypoint on every start).
...
# Labels
LABEL maintainer="Smart Tutor Team" \
      description="Smart Tutor: AI-Powered Personalized Learning Assistant"
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    NODE_ENV=production \
    SMARTTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1
...
# (system.json, default on), exported to SMARTTUTOR_SANDBOX_ALLOW_SUBPROCESS at
...
# (SMARTTUTOR_SANDBOX_RUNNER_URL), which build_backend() prefers.
...
COPY smarttutor/ ./smarttutor/
COPY smarttutor_cli/ ./smarttutor_cli/
...
# user). Each child (backend/frontend) is dropped to this `smarttutor` user via
# the per-program `user=smarttutor` directive, so the app processes stay
# non-root in either runtime. UID 1000 also matches the host user under
# keep-id with a bind mount on ./data.
RUN groupadd --system --gid 1000 smarttutor \
    && useradd --system --uid 1000 --gid 1000 --no-create-home --shell /usr/sbin/nologin smarttutor \
    && chown -R smarttutor:smarttutor /app/data /app/web/.next
...
# smarttutor user (UID 1000) via per-program `user=smarttutor`; see the note on
...
WS_MAX_SIZE=$(python -c "from smarttutor.services.config import get_ws_max_size; print(get_ws_max_size())" 2>/dev/null || echo 16777216)
KEEP_ALIVE=$(python -c "from smarttutor.services.config import HTTP_KEEP_ALIVE_TIMEOUT; print(HTTP_KEEP_ALIVE_TIMEOUT)" 2>/dev/null || echo 300)
exec python -m uvicorn smarttutor.api.main:app --host ${BACKEND_HOST} --port ${BACKEND_PORT} --no-access-log --ws-max-size ${WS_MAX_SIZE} --timeout-keep-alive ${KEEP_ALIVE}
EOF
...
# the configured backend at request time, reading SMARTTUTOR_API_BASE_URL
# (exported by the entrypoint from data/user/settings/system.json).
RUN cat > /app/start-frontend.sh <<'EOF'
#!/bin/bash
set -e

FRONTEND_PORT=${FRONTEND_PORT:-${PORT:-3782}}
...
export SMARTTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1
...
for key in \
    ...
    SMARTTUTOR_API_BASE_URL \
    SMARTTUTOR_AUTH_ENABLED; do
    unset "$key"
done
...
from smarttutor.services.setup import init_user_directories
init_user_directories(Path('/app'))
" 2>/dev/null || echo "   ⚠️ Directory initialization skipped (will be created on first use)"

# Idempotent: re-chown /app/data so the unprivileged `smarttutor` user (UID 1000)
# owns it. Cheap on no-op; the only first-start cost is one stat per file.
chown -R smarttutor:smarttutor /app/data 2>/dev/null || true

echo "⚙️  Loading runtime JSON settings..."
eval "$(python - <<'PY'
import shlex
from smarttutor.services.config import export_runtime_settings_to_env
...
# SMARTTUTOR_API_BASE_URL and SMARTTUTOR_AUTH_ENABLED are exported by the
# export_runtime_settings_to_env eval above (see render_environment in
# smarttutor/services/config/runtime_settings.py). web/proxy.ts reads them at
...
# `smarttutor start` paths stay in sync.
echo "📌 API Base URL (proxy): ${SMARTTUTOR_API_BASE_URL:-http://localhost:${BACKEND_PORT}}"
echo "📌 Auth enabled: ${SMARTTUTOR_AUTH_ENABLED:-false}"
...
# and rootless-keep-id runtimes; children drop to the smarttutor user via
...
COPY --chown=smarttutor:smarttutor --from=frontend-builder /app/web ./web

# `next dev` runs as the unprivileged smarttutor user (via `user=smarttutor` in
...
RUN rm -rf /app/web/.next \
    ...
    && chown smarttutor:smarttutor /app/web /app/web/.next
...
RUN cat > /etc/supervisor/conf.d/programs.conf <<'EOF'
[program:backend]
command=/bin/bash -c "exec python -m uvicorn smarttutor.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT:-8001} --reload --no-access-log --ws-max-size $(python -c 'from smarttutor.services.config import get_ws_max_size; print(get_ws_max_size())' 2>/dev/null || echo 16777216) --timeout-keep-alive $(python -c 'from smarttutor.services.config import HTTP_KEEP_ALIVE_TIMEOUT; print(HTTP_KEEP_ALIVE_TIMEOUT)' 2>/dev/null || echo 300)"
directory=/app
user=smarttutor
autostart=true
autorestart=true
