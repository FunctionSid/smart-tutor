# SmartTutor Containerization

This document covers deploying SmartTutor from a container image: the
...
The published `ghcr.io/hkuds/smarttutor` image runs both the FastAPI
...
- `web/proxy.ts` catches `/api/*` and `/ws/*` and rewrites them to
  `SMARTTUTOR_API_BASE_URL` at request time. That env var is set by the
...
- `supervisord` runs as root (PID 1) and drops each program (backend,
  frontend) to a non-root `smarttutor` user (UID 1000) via its per-program
...
```bash
docker run --rm --name smarttutor \
  -p 127.0.0.1:3782:3782 \
  -v smarttutor-data:/app/data \
  ghcr.io/hkuds/smarttutor:latest
```
...
memory, and knowledge bases persist in the `smarttutor-data` named volume.
...
- **Only `3782` needs to be published.** The browser talks exclusively to
  ...
  middleware (`web/proxy.ts`), which reads `SMARTTUTOR_API_BASE_URL`
...
- **Detached:** add `-d`, then `docker logs -f smarttutor` to follow,
  `docker stop smarttutor` to stop, `docker rm smarttutor` before reusing
  the name. The `smarttutor-data` volume keeps your settings and workspace
  across restarts.
...
python scripts/docker_compose.py \
  ...
  up -d --force-recreate smarttutor
...
without `compose.codex-oauth.yaml`; keep `--force-recreate smarttutor` so the
...
running container, or the empty host directories shadow it and SmartTutor
regenerates the auth secret (logging everyone out) and starts with no
non-admin accounts:

```bash
for tree in system users partners cli-apps; do
  docker cp "smarttutor:/app/data/$tree" "./data/$tree" 2>/dev/null || true
done
```

Deployments using a named volume (`-v smarttutor-data:/app/data`) or the
...
(`https://smarttutor.example.com`), and the in-container Next.js middleware
...
on the host (inside the `smarttutor-data` volume — `docker volume inspect
smarttutor-data` to find its mountpoint) and set the in-network address the
...
`SMARTTUTOR_API_BASE_URL` for `proxy.ts` (precedence: `next_public_api_base`,
then `next_public_api_base_external`, then `http://localhost:8001`). Note
that because the proxy is **server-side**, `SMARTTUTOR_API_BASE_URL` is the
...
SmartTutor permits normal HTTP/HTTPS browser origins by default. With
auth enabled, add exact frontend origins:

```json
{
  "cors_origins": ["https://smarttutor.example.com"]
}
```
...
point. It pulls the same `ghcr.io/hkuds/smarttutor:latest` image and
...
podman compose -f compose.yaml logs -f smarttutor
```
...
- **`userns_mode: keep-id`.** The container's UID 0 maps to your host
  UID; the container's UID 1000 (the `smarttutor` user inside the image)
...
podman run --rm -d --name smarttutor \
...
non-root `smarttutor` user (UID 1000) — `podman exec smarttutor ps -o user,pid,comm`
shows the `uvicorn`/`node` children as `smarttutor`. `supervisord` itself
...
programs are dropped to `smarttutor` via the per-program `user=` directives.
...
- **`system.json` → `next_public_api_base`** (in-network) and
  **`next_public_api_base_external`** (cloud/external override). The
  entrypoint reads these and exports `SMARTTUTOR_API_BASE_URL`, which
...
service up alongside the main `smarttutor` service. With it running, the
...
always a backend that failed to start (check `docker logs smarttutor` for the
`[program:backend]` lines) or a wrong `SMARTTUTOR_API_BASE_URL` in a split
...
`ghcr.io/hkuds/smarttutor:latest` again.
...
- The image drops privileges to a non-root `smarttutor` user (UID 1000)
...
- Auth (`data/user/settings/auth.json` → `auth_enabled = true`) gates
  ...
  `SMARTTUTOR_AUTH_ENABLED` (exported by the entrypoint on every start)
