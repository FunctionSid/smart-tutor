...
CLI-only Smart Tutor distribution. It installs the `smarttutor` command and the
...
FastAPI/Uvicorn server dependencies used by `smarttutor start`.
...
the `smarttutor` command at these source files.
