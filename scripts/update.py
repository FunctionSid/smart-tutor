#!/usr/bin/env python
"""Safely update a local SmartTutor git checkout.
...
def run_update(repo_root: Path, *, assume_yes: bool) -> int:
    ...
    print("Restart SmartTutor if it is currently running.")
    return 0


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch, review, and fast-forward update a local SmartTutor checkout."
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument(
        ...
        help="Path to the git checkout to update. Defaults to this SmartTutor repository.",
    )
