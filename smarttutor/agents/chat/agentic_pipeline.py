...
from smarttutor.agents._shared.tool_composition import (
    ToolMountFlags,
...
from smarttutor.agents.chat.agent_loop import AgentLoop
from smarttutor.agents.chat.context_budget import LLMRequestSnapshot, build_context_budget
from smarttutor.agents.chat.prompt_blocks import ChatPromptAssembler
from smarttutor.capabilities import (
    LoopCapability,
    PromptBlock,
    active_loop_capabilities,
    any_exclusive_capability_active,
)
from smarttutor.capabilities.protocol import END_LOOP
from smarttutor.core.agentic import (
    DispatchOutcome,
    LLMClientConfig,
    UsageTracker,
...
from smarttutor.core.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from smarttutor.core.context import UnifiedContext
from smarttutor.core.stream_bus import StreamBus
from smarttutor.core.tool_protocol import ToolLookup
from smarttutor.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from smarttutor.knowledge.manifest import KbManifest, render_manifest_note
from smarttutor.runtime.providers import ToolScope
from smarttutor.runtime.providers.view import ProviderToolView, build_tool_view
from smarttutor.runtime.registry.deferred_tools import DeferredToolLoader
from smarttutor.runtime.registry.tool_registry import get_tool_registry
from smarttutor.services.cli_apps.models import TOOL_PREFIX as CLI_APP_TOOL_PREFIX
from smarttutor.services.config import get_chat_params
from smarttutor.services.llm import (
    get_llm_config,
    get_token_limit_kwargs,  # noqa: F401  (re-exported for tests)
    prepare_multimodal_messages,
    supports_tools,  # noqa: F401  (re-exported for tests)
)
from smarttutor.services.llm.context_window import resolve_effective_context_window
from smarttutor.services.prompt import get_prompt_manager
from smarttutor.tools.builtin import PARTNER_BUILTIN_TOOL_NAMES
...
_GENERATION_TOOL_SERVICES: dict[str, str] = {}
...
def _drop_unconfigured_generation_tools(tools: list[str]) -> list[str]:
    ...
    if not present:
        return tools
    try:
        from smarttutor.services.config.model_catalog import get_model_catalog_service
...
class AgenticChatPipeline:
    ...
    async def _pageindex_sdk_tool_bundles(self, context: UnifiedContext):
        from smarttutor.multi_user.knowledge_access import resolve_kb
        from smarttutor.services.rag.factory import PAGEINDEX_OSS_PROVIDER, PAGEINDEX_PROVIDER
        from smarttutor.services.rag.pipelines.pageindex.tools import build_sdk_tool_bundle
        from smarttutor.services.rag.provider_binding import resolve_bound_provider
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from smarttutor.services.sandbox import IsolationLevel, get_sandbox_service
            ...
            if level is IsolationLevel.SYSTEM:
                ...
                from smarttutor.multi_user.tool_access import exec_override
    ...
    def _notebook_choices_full() -> list[dict[str, Any]]:
        try:
            from smarttutor.services.notebook import get_notebook_manager
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        from smarttutor.core.agentic import execute_tool_call
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag":
            ...
            from smarttutor.services.sandbox import Mount
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        from smarttutor.services.session.context_builder import count_tokens
    ...
    def _current_owner_id() -> str:
        ...
        """
        try:
            from smarttutor.multi_user.paths import current_owner_id
    ...
    def _rag_kbs(self, context: UnifiedContext) -> list[str]:
        ...
        from smarttutor.services.rag.pipelines.pageindex import is_pageindex_kb
    ...
    def _collect_kb_manifests(kbs: list[str]) -> list[KbManifest]:
        from smarttutor.multi_user.knowledge_access import resolve_kb_manifest
