...
The main entry point is :class:`~smarttutor.agents.question.pipeline.QuestionPipeline`.
...
def __getattr__(name: str) -> Any:
    if name == "AgentCoordinator":
        module = import_module("smarttutor.agents.question.coordinator")
        return getattr(module, name)
    if name == "FollowupAgent":
        module = import_module("smarttutor.agents.question.agents.followup_agent")
        return getattr(module, name)
    if name in {"QuestionPipeline", "QuizTemplate", "QuizPair", "QuizPlan", "QuizHistoryEntry"}:
        module = import_module("smarttutor.agents.question.pipeline")
        return getattr(module, name)
