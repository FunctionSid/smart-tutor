...
from smarttutor.core.agentic.usage import UsageTracker
from smarttutor.core.capability_protocol import BaseCapability, CapabilityManifest
from smarttutor.core.context import UnifiedContext
from smarttutor.core.stream_bus import StreamBus
from smarttutor.core.trace import merge_trace_metadata
from smarttutor.i18n import StatusI18n
from smarttutor.runtime.request_contracts import get_capability_request_schema
...
class DeepQuestionCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if isinstance(followup_question_context, dict) and followup_question_context.get(
            ...
            from smarttutor.agents.question.agents.followup_agent import FollowupAgent
        ...
        if mode != "mimic":
            ...
            from smarttutor.agents.question.history import load_session_quiz_history
            from smarttutor.agents.question.pipeline import QuestionPipeline
            from smarttutor.agents.question.request_config import (
                build_question_runtime_config,
            )
            from smarttutor.services.config import load_config_with_main
    ...
    async def _run_mimic_mode(
        ...
        * ``[Attached Documents]`` in  → no paper available; fall back to
        ...
        """
        from smarttutor.agents.question.history import load_session_quiz_history
        from smarttutor.agents.question.mimic_source import (
        ...
        from smarttutor.services.config import load_config_with_main
        from smarttutor.services.parsing.engines.mineru.config import MinerUError
