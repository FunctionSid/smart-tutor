...
* :func:`load_session_quiz_history` — async, takes ``session_id`` and an
  ...
  :class:`~smarttutor.agents.question.pipeline.QuizHistoryEntry`.
...
from smarttutor.agents.question.pipeline import QuizHistoryEntry
...
async def load_session_quiz_history(
    ...
    *,
    ...
    if not session_id or max_entries <= 0:
        return []
    try:
        from smarttutor.services.session.sqlite_store import get_sqlite_session_store
