...
from smarttutor.runtime.request_contracts import (
...
class VisualizeCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        from smarttutor.agents.visualize.models import ReviewResult
        from smarttutor.agents.visualize.pipeline import VisualizePipeline
        from smarttutor.agents.visualize.utils import (
            build_fallback_html,
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'smarttutor[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from smarttutor.agents.math_animator.pipeline import MathAnimatorPipeline
        from smarttutor.agents.math_animator.request_config import MathAnimatorRequestConfig
        from smarttutor.core.trace import build_trace_metadata, new_call_id
        from smarttutor.services.llm.config import get_llm_config
