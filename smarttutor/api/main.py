...
from smarttutor.logging import configure_logging
...
def validate_tool_consistency():
    ...
    """
    try:
        from smarttutor.runtime.registry.capability_registry import get_capability_registry
...
async def lifespan(app: FastAPI):
    ...
    try:
        from smarttutor.services.llm import get_llm_client
    ...
    try:
        from smarttutor.events.event_bus import get_event_bus
    ...
    try:
        from smarttutor.services.partners import get_partner_manager
    ...
    try:
        from smarttutor.services.cron import get_cron_service
    ...
    try:
        from smarttutor.services.pocketbase_client import ping_pocketbase
    ...
    try:
        from smarttutor.services.memory import (
    ...
    try:
        from smarttutor.services.mcp import get_mcp_manager
    ...
    try:
        from smarttutor.services.llm.provider_factory import close_runtime_provider_pool
    ...
    try:
        from smarttutor.core.agentic.client import close_agentic_client_pool
...
app = FastAPI(
    ...
    # Prevent HTTPS-to-HTTP downgrade surprises behind reverse proxies.
...
# ones worth seeing.
#
# The `smarttutor.access` logger gets its own INFO stdout handler rather than
...
_access_logger = logging.getLogger("smarttutor.access")
if not any(getattr(h, "_smarttutor_access_handler", False) for h in _access_logger.handlers):
    ...
    _access_handler._smarttutor_access_handler = True  # type: ignore[attr-defined]
...
from smarttutor.multi_user.router import router as multi_user_router  # noqa: E402
...
from smarttutor.api.routers.auth import require_admin, require_auth  # noqa: E402
...
if __name__ == "__main__":
    from smarttutor.api.run_server import main as run_server_main
