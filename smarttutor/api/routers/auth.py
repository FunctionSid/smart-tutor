...
from smarttutor.services.config import load_auth_settings
...
from smarttutor.multi_user.context import set_current_user, user_from_token_payload
from smarttutor.multi_user.paths import local_admin_user
from smarttutor.services.auth import (
    AUTH_ENABLED,
    POCKETBASE_ENABLED,
    TOKEN_EXPIRE_HOURS,
    TokenPayload,
...
from smarttutor.services.codex_auth.service import deliver_codex_oauth_callback
...
def _local_admin_token_payload() -> TokenPayload:
    ...
    ``smarttutor/multi_user/paths.py``.
    """
    from smarttutor.multi_user.models import LOCAL_ADMIN_ID, LOCAL_ADMIN_USERNAME
...
async def receive_codex_oauth_callback(
    ...
    try:
        ...
        return HTMLResponse(
            (
                "<!doctype html><title>SmartTutor Codex</title>"
                "<p>Authentication could not be received. Return to SmartTutor and try again.</p>"
            ),
    ...
    return HTMLResponse(
        (
            "<!doctype html><title>SmartTutor Codex</title>"
            "<p>Authentication received. You can return to SmartTutor.</p>"
        ),
        headers=headers,
    )
...
async def update_profile(
    ...
    from smarttutor.multi_user.identity import delete_avatar_file
...
async def upload_avatar(
    ...
    from smarttutor.multi_user.identity import save_avatar_file
...
async def get_avatar_image(
    ...
    from smarttutor.multi_user.identity import get_avatar_file
