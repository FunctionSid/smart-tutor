...
from smarttutor.api.utils.progress_broadcaster import ProgressBroadcaster
from smarttutor.api.utils.task_id_manager import TaskIDManager
from smarttutor.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from smarttutor.knowledge.add_documents import DocumentAdder, remove_raw_document
from smarttutor.knowledge.initializer import KnowledgeBaseInitializer
from smarttutor.knowledge.kb_types import is_connected_kb, supports_local_raw_files
from smarttutor.knowledge.manager import KnowledgeBaseManager
from smarttutor.knowledge.naming import validate_knowledge_base_name
from smarttutor.knowledge.progress_tracker import ProgressStage, ProgressTracker
from smarttutor.logging import PROCESS_LOG_PRIVATE_ATTR
from smarttutor.multi_user.context import get_current_user
from smarttutor.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from smarttutor.services.config import PROJECT_ROOT, load_config_with_main
from smarttutor.services.file_io import atomic_write_json
from smarttutor.services.rag.factory import (
    DEFAULT_PROVIDER,
    GRAPHRAG_PROVIDER,
    LIGHTRAG_PROVIDER,
...
from smarttutor.services.rag.file_routing import FileTypeRouter
from smarttutor.services.rag.linked_kb import (
    LINKABLE_PROVIDERS,
    assert_path_allowed,
    probe_linked_folder,
)
from smarttutor.services.rag.pipelines.ima.client import (
    MAX_PAGE_LIMIT,
    ImaAPIError,
    ImaAuthError,
    ImaClient,
...
from smarttutor.services.rag.pipelines.ima.config import (
    ImaConfig,
    ImaCredentials,
    get_account_credentials,
)
from smarttutor.utils.document_extractor import (
    MAX_EXTRACTED_CHARS_PER_DOC,
    DocumentExtractionError,
    extract_text_from_path,
)
from smarttutor.utils.document_validator import DocumentValidator
from smarttutor.utils.error_utils import format_exception_message
...
def _save_zip_archive(
    ...
    from smarttutor.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from smarttutor.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from smarttutor.services.pocketbase_client import get_pb_client
...
def _assert_provider_ready(provider: str) -> None:
    ...
    if provider == PAGEINDEX_PROVIDER:
        from smarttutor.services.rag.pipelines.pageindex.config import is_pageindex_configured
    ...
    if provider == PAGEINDEX_OSS_PROVIDER:
        from smarttutor.services.rag.preflight import engine_preflight
    ...
    if provider == GRAPHRAG_PROVIDER:
        from smarttutor.services.rag.pipelines.graphrag.config import is_graphrag_available
        ...
        if not is_graphrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'smarttutor[graphrag]'` on the server before "
    ...
    if provider == LIGHTRAG_PROVIDER:
        from smarttutor.services.rag.pipelines.lightrag.config import is_lightrag_available
        ...
        if not is_lightrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'smarttutor[rag-lightrag]'` on the server before "
...
def _enforce_provider_formats(provider: str, files: list[UploadFile]) -> None:
    ...
    if provider not in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        return
    from smarttutor.services.rag.pipelines.pageindex.pipeline import (
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from smarttutor.services.rag.index_probe import inspect_provider_version
        from smarttutor.services.rag.pipelines.llamaindex.storage import (
...
async def run_upload_processing_task(
    ...
    with capture_task_logs(task_id):
        try:
            ...
            staging = adder.last_staging_summary
            if staging.duplicate_count:
                _task_log(
                    task_id,
                    f"Skipped {staging.duplicate_count} duplicate file(s) already in the index",
                )
            if staging.missing_count:
                _task_log(
                    task_id,
                    f"Skipped {staging.missing_count} missing or invalid file(s)",
                    level="warning",
                )
            progress_tracker.update(
                ProgressStage.PROCESSING_DOCUMENTS,
                message_key=(
                    ...
                    else "Staged {{count}} new file(s)"
                ),
                message_params={
                    "count": len(staged_files),
                    "skipped": staging.skipped_count,
                },
                current=0,
                total=len(staged_files),
                skipped_count=staging.skipped_count,
                duplicate_count=staging.duplicate_count,
            )
...
async def get_rag_providers():
    ...
    try:
        from smarttutor.services.config import get_kb_config_service
        from smarttutor.services.rag.service import RAGService
...
def _pageindex_config_payload() -> dict:
    ...
    from smarttutor.services.config import get_runtime_settings_service
...
def _model_options_payload(kinds: list[str]) -> dict:
    ...
    """
    from smarttutor.services.config import get_model_catalog_service
...
async def _probe_graphrag_model_compatibility(profile_id: str, model_id: str) -> dict:
    ...
    from smarttutor.services.rag.pipelines.graphrag.compatibility import (
...
async def update_kb_config(kb_name: str, config: dict):
    ...
    try:
        from smarttutor.services.config import get_kb_config_service
        from smarttutor.services.rag.index_probe import has_ready_provider_index
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    """
    from smarttutor.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def connect_lightrag_server_route(payload: ConnectLightRagServerRequest):
    ...
    """
    from smarttutor.services.rag.pipelines.lightrag_server.config import SUPPORTED_MODES
    from smarttutor.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def probe_ima_route(payload: ProbeImaRequest):
    ...
    Creates nothing.
    """
    from smarttutor.services.rag.pipelines.ima.probe import probe_knowledge_base
...
def _resolve_kb_raw_dir(kb_name: str, *, allow_unsupported: bool = False) -> Path | None:
    ...
    SmartTutor-managed raw directory.  File listing treats that as an empty
...
async def delete_kb_file(kb_name: str, filename: str):
    ...
    if provider in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        from smarttutor.services.rag.factory import get_pipeline
...
async def run_reindex_task(kb_name: str, base_dir: str, task_id: str, signature_hash: str) -> None:
    ...
    with capture_task_logs(task_id):
        try:
            ...
            # provider=None → RAGService resolves the KB's SmartTutor-bound
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        if provider_uses_embedding_versions(kb_provider):
            from smarttutor.services.rag.embedding_signature import signature_from_embedding_config
            from smarttutor.services.rag.index_versioning import (
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from smarttutor.api.routers.auth import ws_auth_failed, ws_require_auth
    from smarttutor.multi_user.context import reset_current_user
