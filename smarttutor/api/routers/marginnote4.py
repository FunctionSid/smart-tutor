...
to pair with this SmartTutor instance and push synced study data.

Authentication layers:
* ``/pair``, ``/devices``, ``/status`` -- SmartTutor session auth (the logged-in
...
from smarttutor.api.routers.auth import require_auth
from smarttutor.capabilities.marginnote4.models import MarginNoteObject, SyncBatch
from smarttutor.capabilities.marginnote4.store import MarginNoteStore, resolve_db_path
...
# -- session-authenticated endpoints (SmartTutor user) -----------------------
...
async def pair_device(body: PairRequest, request: Request) -> PairResponse:
    """Pair a new MN4 device. Requires a SmartTutor session.
