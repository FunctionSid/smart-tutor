...
async def resolve_entry(entry_id: str):
    ...
    from smarttutor.services.memory.document import parse
...
async def reset_doc(layer: str, key: str):
    ...
    from smarttutor.services.memory.consolidator.runs import get_run_manager
    ...
    from smarttutor.services.memory.consolidator import meta as meta_mod
...
def _runner_for(req: RunStartRequest):
    ...
    from smarttutor.services.memory.consolidator import (
        run_audit,
        run_dedup,
        run_merge,
        run_update,
...
async def start_run(req: RunStartRequest):
    ...
    from smarttutor.services.memory.consolidator.runs import (
        RunBusyError,
        get_run_manager,
    )
...
async def get_doc_lines(layer: str, key: str):
    ...
    from smarttutor.services.memory.consolidator.line_doc import render_view
    from smarttutor.services.memory.document import Document, parse
...
async def get_memory_settings_endpoint():
    ...
    from smarttutor.services.memory.settings import memory_settings_dict
...
async def get_trace(surface: str, limit: int = 200, offset: int = 0):
    ...
    from smarttutor.services.memory.trace import iter_since
