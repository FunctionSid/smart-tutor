...
from smarttutor.core.i18n import t
from smarttutor.partners.config.paths import get_partner_media_dir
from smarttutor.partners.helpers import safe_filename
from smarttutor.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
from smarttutor.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    from smarttutor.partners.config.schema import ChannelsConfig
...
def _validate_llm_selection_payload(
    ...
    from smarttutor.services.model_selection import apply_llm_selection_to_catalog
    from smarttutor.services.partners.model_runtime import normalize_partner_llm_selection
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from smarttutor.services.partners.workspace import DEFAULT_SOUL
...
async def list_channel_schemas():
    ...
    from smarttutor.api.routers._partners_channel_schema import all_channel_schemas
...
async def tool_options():
    ...
    """
    from smarttutor.agents._shared.tool_composition import admin_enabled_optional_tools
    from smarttutor.api.utils.tool_options import build_tool_options
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a SmartTutor restart (a manual /stop clears
...
async def partner_command_palette():
    from smarttutor.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from smarttutor.services.config.runtime_settings import get_chat_attachment_limits
...
async def _partner_chat_stream(
    ...
    from smarttutor.core.stream import StreamEventType
