...
from smarttutor.i18n.metadata_i18n import tool_description_i18n
...
def _discover_plugins() -> list[Any]:
    try:
        from smarttutor.plugins.loader import discover_plugins
    except Exception:
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    stdout_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="smarttutor.playground.stdout"
    )
    stderr_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="smarttutor.playground.stderr"
    )
...
async def _execute_capability_stream(
    ...
    if capability_name == "chat" and partner_id:
        from smarttutor.api.routers.partners import (
            ChatMessageRequest,
