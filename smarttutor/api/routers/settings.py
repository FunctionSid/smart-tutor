...
from smarttutor.multi_user.model_access import allowed_llm_options
from smarttutor.services.codebuddy_auth import get_codebuddy_auth_service
from smarttutor.services.codex_auth import (
    CodexAuthError,
...
from smarttutor.services.config.origins import normalize_origins
from smarttutor.services.config.runtime_settings import (
    CHAT_ATTACHMENT_CHARS_RANGE,
    CHAT_ATTACHMENT_MAX_FILE_MB_RANGE,
    CHAT_ATTACHMENT_MAX_TOTAL_MB_RANGE,
    compute_ws_max_size,
)
from smarttutor.services.embedding.client import reset_embedding_client
from smarttutor.services.llm.client import reset_llm_client
from smarttutor.services.llm.config import clear_llm_config_cache
from smarttutor.services.model_selection import list_llm_options
from smarttutor.services.path_service import get_path_service
from smarttutor.services.settings.interface_settings import (
...
from smarttutor.services.settings.starter_settings import (
    TRACE_COUNT_RANGE as STARTER_TRACE_COUNT_RANGE,
)
from smarttutor.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
def _ollama_profile_base_url(profile: dict[str, Any]) -> str:
    from smarttutor.services.provider_registry import find_by_name

    spec = find_by_name("ollama")
    return (str(profile.get("base_url") or "").strip() or (spec.default_api_base if spec else "")).rstrip(
        "/"
    )
...
async def _refresh_ollama_llm_profiles(
    ...
    *,
    ...
    for profile in profiles:
            ...
            statuses.append(
                {
                    ...
                    "provider": "ollama",
                    "ok": False,
                    "message": f"No Ollama models found at {base_url}.",
                }
            )
            continue
        ...
        next_models = [
            _catalog_model_entry(model_name, existing_by_model.get(model_name), index)
...
def get_enabled_optional_tools() -> list[str]:
    ...
    """
    from smarttutor.multi_user.tool_access import allowed_optional_tools
...
def _require_codex_oauth_actor() -> None:
    ...
    """
    from smarttutor.services.partners.scope import is_partner_user_id
...
def _provider_choices() -> dict[str, list[dict[str, Any]]]:
    ...
    from smarttutor.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from smarttutor.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from smarttutor.services.parsing.engines._install import (
        installable_engines,
        model_downloadable_engines,
    )
    from smarttutor.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from smarttutor.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from smarttutor.services.parsing.engines.factory import get_parser, is_engine_available
...
async def test_docling_remote_connection(payload: DoclingRemoteTest):
    ...
    is untouched."""
    _require_settings_admin()
    from smarttutor.services.parsing.engines.docling.config import (
        DoclingConfig,
        resolve_docling_config,
    )
    from smarttutor.services.parsing.engines.docling.remote import verify_remote
...
async def test_tika_remote_connection(payload: TikaRemoteTest):
    ...
    _require_settings_admin()
    from smarttutor.services.parsing.engines.tika.config import TikaConfig
    from smarttutor.services.parsing.engines.tika.remote import verify_remote
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install smarttutor[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def document_parsing_job_status(cursor: int = 0):
    _require_settings_admin()
    from smarttutor.services.parsing.engines._install import get_background_job_manager
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from smarttutor.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from smarttutor.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from smarttutor.services.parsing.engines.mineru.cloud import verify_credentials
    from smarttutor.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from smarttutor.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def get_llm_options(refresh_local: bool = False, force_refresh: bool = False):
    ...
    if local_statuses:
        payload["local_statuses"] = local_statuses
    return payload
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    if not base_url:
        ...
        if spec and spec.is_local and spec.default_api_base:
            base_url = spec.default_api_base
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. SmartTutor will restart shortly.",
        "launch_at": launch_at,
