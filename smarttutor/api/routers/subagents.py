...
from smarttutor.api.routers.auth import require_admin
from smarttutor.knowledge.kb_types import SUBAGENT_KB_TYPE
from smarttutor.multi_user.knowledge_access import current_kb_manager
from smarttutor.multi_user.partner_access import assert_partner_allowed, visible_partner_cards
from smarttutor.services.rag.linked_kb import assert_path_allowed
from smarttutor.services.subagent import (
...
async def backend_options():
    ...
    from smarttutor.services.subagent.models import list_backend_options
...
async def sync_backend(kind: str):
    ...
    """
    from smarttutor.services.subagent import get_backend
    from smarttutor.services.subagent.models import sync_backend_options
...
async def create_connection(payload: ConnectSubagentRequest):
    ...
    consult within one SmartTutor chat lands in that one partner session.
    """
...
async def delete_connection(name: str):
    ...
    from smarttutor.services.subagent.sessions import forget_connection
...
async def message_connection(name: str, payload: SubagentMessageRequest):
    ...
    live session SmartTutor consults (shared via the cross-turn registry, keyed by
    ...
    from smarttutor.services.subagent import get_backend
    from smarttutor.services.subagent.sessions import get_session, remember_session, session_key
