"""
Shared prompt loader for non-BaseAgent call sites in ``smarttutor/book``.

All LLM-facing prompts inside this module live as YAML under
``smarttutor/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~smarttutor.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``smarttutor/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected smarttutor/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
