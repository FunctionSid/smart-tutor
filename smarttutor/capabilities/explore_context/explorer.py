...
* **Fallback** (:meth:`_single_pass`) — for providers without native tool
  ...
  single pass, modelled on :class:`~smarttutor.agents.notebook.analysis_agent`.
"""
...
from smarttutor.services.llm import clean_thinking_tags, get_llm_config, get_token_limit_kwargs
...
class ContextExplorer:
    ...
    async def _call_llm(
        ...
        if threads_session_id(self.binding):
            kwargs["smarttutor_session_id"] = f"{session_id}:explore"
        if tool_schemas:
