...
(see :class:`~smarttutor.capabilities.ima.capability.ImaCapability`). They cover
...
from smarttutor.capabilities.ima.binding import ImaBinding, resolve_client, select_binding
...
# this module is imported while ``smarttutor.tools.builtin`` builds the global tool
# table, and reaching into ``smarttutor.services.rag`` at import time would close
...
def _note_sort(name: Any) -> int:
    ...
    from smarttutor.services.rag.pipelines.ima.notes import (
        SORT_BY_CREATED,
        SORT_BY_TITLE,
        SORT_BY_UPDATED,
    )
...
class _ImaTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from smarttutor.services.rag.pipelines.ima.envelope import (
            ImaAPIError,
            ImaAuthError,
            ImaRateLimitError,
        )
...
class ImaReadTool(_ImaTool):
    ...
    async def _run(self, client, binding, kwargs):
        from smarttutor.services.rag.pipelines.ima import media as media_ops
        from smarttutor.services.rag.pipelines.ima import sources as source_policy
