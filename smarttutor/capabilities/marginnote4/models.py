...
These types describe MN4 objects as they land in SmartTutor after sync.
...
* ``raw`` preserves the original MN4 JSON for fields SmartTutor does not model
...
class MarginNoteObject:
    ...
    Attributes:
        ...
        synced_at:  ISO-8601 timestamp when SmartTutor received this version.
...
@dataclass(slots=True)
class PairedDevice:
    """A MarginNote 4 device registered with this SmartTutor instance.
...
class LearningEvent:
    ...
    Captures a review or study interaction so SmartTutor can track mastery
