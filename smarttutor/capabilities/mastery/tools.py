...
mastery engine (:mod:`smarttutor.learning`).
...
from smarttutor.capabilities.mastery.choices import (
    format_options,
    has_option_bodies,
    parse_options,
...
from smarttutor.learning.pending import public_pending_question
from smarttutor.learning.policy import (
    QUALITATIVE_TYPES,
    display_mastery,
...
if TYPE_CHECKING:
    from smarttutor.learning.models import LearningProgress
...
class MasteryQuizTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from smarttutor.learning.service import MasteryInteractionError
...
class MasteryGradeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not path_id:
            return _no_path_result()
        from smarttutor.learning.scheduler import SpacedRepetitionScheduler
...
class MasterySwitchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.capabilities.mastery.binding import (
            PathBindingError,
            rebind_active_path,
        )
