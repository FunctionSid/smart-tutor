...
*Personal* rows are the ones :class:`~smarttutor.services.path_service.PathService`
...
:func:`smarttutor.api.routers.settings._require_settings_admin` already guards —
...
(:func:`~smarttutor.multi_user.paths.get_owner_path_service`), so honouring a
configuration request from an IM-facing companion would mean letting whoever is
chatting with it reconfigure the owner's SmartTutor — including, if the owner is
...
from smarttutor.services.config.settings_spec import Scope
    ...
    if _is_partner_turn():
        return AccessDecision(
            ...
            reason=(
                ...
                "The owner can make this change in SmartTutor directly."
            ),
        )
    if scope == "personal":
