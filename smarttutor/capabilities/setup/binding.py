...
def setup_gaps() -> tuple[SetupGap, ...]:
    ...
    """
    from smarttutor.services.config.settings_spec import setting_specs
...
# configuring SmartTutor itself.
_DIRECT_PATTERN = re.compile(
    r"(帮我配置|帮我设置|自己配置|配置一下\s*smarttutor|设置一下\s*smarttutor|初始化配置)"
    r"|(\bconfigure\s+smarttutor\b|\bset\s?up\s+smarttutor\b|\bsetup\s+wizard\b)",
    re.IGNORECASE,
)
