"""The four tools through which SmartTutor configures itself.

They are deliberately generic: each one walks
:mod:`smarttutor.services.config.settings_spec` rather than knowing anything
...
``apply_setting``
    Commit one row, after the checks in :mod:`smarttutor.capabilities.setup.apply`.
...
class InspectSetupTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="inspect_setup",
            description=(
                "Read SmartTutor's own configuration: every setting you can "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.capabilities.setup.access import can_write
        from smarttutor.capabilities.setup.binding import setup_gaps
        from smarttutor.capabilities.setup.jobs import available_jobs
        from smarttutor.services.config.settings_spec import specs_for_area
...
class ApplySettingTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="apply_setting",
            description=(
                "Change one SmartTutor setting. The value must be one of the "
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "entered directly into SmartTutor. Call it, tell the user what "
