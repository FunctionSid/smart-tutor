...
(resolved by :mod:`smarttutor.capabilities.subagent.binding`). As a
...
from smarttutor.capabilities.protocol import KnowledgeCapability, PromptBlock
from smarttutor.capabilities.subagent.binding import connection_for_turn, subagent_refs
from smarttutor.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from smarttutor.services.subagent import load_subagent_settings
        from smarttutor.services.subagent.sessions import get_session, session_key
        ...
        # session the user/SmartTutor built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by SmartTutor on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from smarttutor.services.subagent import load_subagent_settings
    from smarttutor.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from smarttutor.services.subagent import PARTNER_BACKEND_KIND
