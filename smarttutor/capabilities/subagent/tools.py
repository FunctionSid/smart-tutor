...
:class:`~smarttutor.capabilities.subagent.capability.SubagentCapability`, which
...
if TYPE_CHECKING:  # avoid importing the services package at module-load time (cycle)
    from smarttutor.services.subagent import SubagentEvent
...
class ConsultSubagentTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        # Head this round with the question SmartTutor is putting to the agent, so
        ...
        if result.session_id:
            ...
            if session_key_value:
                from smarttutor.services.subagent.sessions import remember_session
