...
@dataclass(frozen=True)
class DocumentStagingSummary:
    """What happened while validating and staging an add-documents batch."""
...
class DocumentAdder:
    ...
    def __init__(
        ...
        # Incremental adds must use the engine SmartTutor has bound to this KB. An
        ...
        self.last_staging_summary = DocumentStagingSummary()
    ...
    def add_documents(
        ...
        self.last_staging_summary = DocumentStagingSummary(
            staged_count=len(files_to_process),
        ...
        for idx, doc_file in enumerate(new_files, 1):
            try:
                if self.progress_tracker is not None:
                    from smarttutor.knowledge.progress_tracker import ProgressStage
