...
from smarttutor.services.rag.factory import normalize_provider_name
...
class KnowledgeBaseInitializer:
    ...
    def _update_metadata_with_provider(self, provider: str) -> None:
        ...
        raw_files = FileTypeRouter.collect_supported_files(self.raw_dir, recursive=True)
        metadata["last_indexed_count"] = len(raw_files)
        metadata["last_indexed_action"] = "create"
        metadata["file_hashes"] = {
            path.relative_to(self.raw_dir).as_posix(): self._get_file_hash(path)
    ...
    async def display_statistics_generic(self) -> None:
        ...
        from smarttutor.services.rag.index_probe import inspect_kb_versions
