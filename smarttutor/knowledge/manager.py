...
from smarttutor.knowledge.kb_types import (
    IMA_KB_TYPE,
...
from smarttutor.knowledge.manifest import iter_kb_documents
...
from smarttutor.services.rag.index_probe import (
    inspect_kb_versions,
...
def _get_embedding_fingerprint() -> tuple[str, int] | None:
    ...
    try:
        from smarttutor.services.embedding import get_embedding_config
...
class KnowledgeBaseManager:
    ...
    def update_kb_status(
        ...
        if isinstance(progress, dict):
            ...
            raw_skipped_count = progress.get("skipped_count")
            if isinstance(raw_skipped_count, bool):
                skipped_count = int(raw_skipped_count)
            elif isinstance(raw_skipped_count, (int, float)):
                skipped_count = int(raw_skipped_count)
            elif isinstance(raw_skipped_count, str):
                try:
                    skipped_count = int(raw_skipped_count)
                except ValueError:
                    skipped_count = None

            raw_duplicate_count = progress.get("duplicate_count")
            if isinstance(raw_duplicate_count, bool):
                duplicate_count = int(raw_duplicate_count)
            elif isinstance(raw_duplicate_count, (int, float)):
                duplicate_count = int(raw_duplicate_count)
            elif isinstance(raw_duplicate_count, str):
                try:
                    duplicate_count = int(raw_duplicate_count)
                except ValueError:
        ...
        if status == "ready":
            ...
            if progress is not None:
                ...
                if skipped_count is not None:
                    kb_config["last_upload_skipped_count"] = max(skipped_count, 0)
                if duplicate_count is not None:
                    kb_config["last_upload_duplicate_count"] = max(duplicate_count, 0)
        elif status == "error":
            if progress is not None:
        ...
        if status == "ready":
            ...
            try:
                from smarttutor.services.rag.embedding_signature import (
    ...
    def list_knowledge_bases(self) -> list[str]:
        ...
        if base_exists:
            for item in self.base_dir.iterdir():
                ...
                from smarttutor.services.rag.index_versioning import list_kb_versions
    ...
    def _marginnote4_store_owner(
        ...
        from smarttutor.capabilities.marginnote4.store import resolve_db_path
        ...
        *,
        ...
        if dir_exists:
            try:
                ...
                # counts for the same KB (see :mod:`smarttutor.knowledge.manifest`).
