...
from smarttutor.api.routers.mastery_path import router
from smarttutor.learning.models import PendingQuestion
...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "smarttutor.api.routers.mastery_path.LearningStore",
...
class TestGenerateFromNotebook:
    ...
    @patch("smarttutor.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("smarttutor.api.routers.mastery_path.get_response_language", return_value="en")
    @patch("smarttutor.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("smarttutor.api.routers.mastery_path.get_response_language", return_value="zh")
    @patch("smarttutor.services.llm.complete", new_callable=AsyncMock)
