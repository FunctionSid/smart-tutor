...
def test_grade_and_record_blank_wrong_is_metacognitive(tmp_path):
    ...
    from smarttutor.learning.models import ErrorType
...
def test_record_qualitative_starts_at_first_review_interval(
    ...
    now = 1_700_000_000.0
    monkeypatch.setattr("smarttutor.learning.scheduler.time.time", lambda: now)
...
def test_record_qualitative_updates_existing_review_state(tmp_path, monkeypatch):
    ...
    progress = _make_progress()
    now = [1_700_000_000.0]
    monkeypatch.setattr("smarttutor.learning.scheduler.time.time", lambda: now[0])
