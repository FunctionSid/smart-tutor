...
:func:`~smarttutor.multi_user.model_access.is_owner_bound`). Before this
...
:class:`~smarttutor.services.llm.provider_core.openai_codex_provider.OpenAICodexProvider`).

Ownership resolution is shared with the credential store via
:func:`~smarttutor.multi_user.paths.get_owner_path_service`, so a profile and
...
def _codex_profile_is_current(profile: dict[str, Any]) -> bool:
    try:
        from smarttutor.services.codex_auth import get_codex_oauth_service
