...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install smarttutor[matrix]"
    ) from e
...
from smarttutor.partners.config.paths import get_data_dir
from smarttutor.partners.config.schema import DeliveryOverrides
    ...
    async def start(self) -> None:
        ...
        if self.config.e2ee_enabled and not MATRIX_E2EE_AVAILABLE:
            raise ImportError(
                ...
                "`smarttutor[matrix-e2e]` or `requirements/matrix-e2e.txt`, and make "
    ...
    def _decrypt_media_bytes(self, event: MatrixMediaEvent, ciphertext: bytes) -> bytes | None:
        if decrypt_attachment is None:
            logger.warning(
                ...
                "Install smarttutor[matrix-e2e] to decrypt encrypted media."
            )
            return None
        key_obj, hashes, iv = (
