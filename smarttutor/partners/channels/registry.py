...
def discover_channel_names() -> list[str]:
    ...
    import smarttutor.partners.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from smarttutor.partners.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"smarttutor.partners.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in smarttutor.partners.channels.{module_name}")
...
def discover_plugins() -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="smarttutor.partners.channels"):
        try:
            cls = ep.load()
            plugins[ep.name] = cls
