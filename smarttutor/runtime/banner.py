"""Branded banner + localized labels for ``smarttutor start`` / ``smarttutor init``.
...
from smarttutor.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `smarttutor start` to launch SmartTutor.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `smarttutor start` again."
        ),
        ...
        "start.port_in_use": (
            "SmartTutor cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `smarttutor start` 启动 SmartTutor。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `smarttutor start`。"
        ),
...
def resolve_language(default: str = "en") -> str:
    ...
    """
    try:
        from smarttutor.services.settings.interface_settings import get_ui_language
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]SmartTutor[/]",
