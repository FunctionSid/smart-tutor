"""Runtime home resolution for installed and source SmartTutor runs."""
...
SMARTTUTOR_HOME_ENV = "SMARTTUTOR_HOME"
...
def validate_runtime_home(home: str | Path) -> None:
    ...
    if resolved in invalid_homes:
        raise ValueError(
            f"Invalid SmartTutor runtime home: {resolved}. Runtime data must live under "
            f"{PACKAGE_ROOT / 'data'} without nesting. Start SmartTutor with "
            f"SMARTTUTOR_HOME={PACKAGE_ROOT} from {PACKAGE_ROOT}."
        )
...
def get_runtime_home(home: str | Path | None = None) -> Path:
    ...
    2. ``SMARTTUTOR_HOME`` environment variable.
    ...
    raw = home if home is not None else os.getenv(SMARTTUTOR_HOME_ENV)
