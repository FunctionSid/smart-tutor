"""Local Web launcher for the installed SmartTutor app."""
...
SOURCE_PRODUCTION_DIST_DIR = ".next-smarttutor"
SOURCE_BUILD_MARKER = ".smarttutor-build.json"
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected SMARTTUTOR_HOME visible to path/config singletons."""
    try:
...
def _packaged_web_dir() -> Path | None:
    try:
        import smarttutor_web
    except ImportError:
        return None
    path = Path(smarttutor_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".smarttutor-web-runtime.json"
...
def _ensure_web_dependencies(source: Path, npm: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            "Fix the error above, then retry `smarttutor start`."
        )
...
def _source_production_env(*, api_base: str, auth_enabled: bool) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env["SMARTTUTOR_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR
...
def _source_build_fingerprint(source: Path, env: dict[str, str]) -> str:
    ...
    version_file = PACKAGE_ROOT / "smarttutor" / "__version__.py"
...
def _ensure_source_production_build(
    ...
    *,
    ...
    Long-running ``smarttutor start`` processes must not use ``next dev``: its
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged SmartTutor Web app.")
    ...
    raise SystemExit(
        "Smart Tutor Web assets are not installed. Install the full app with `pip install -U smarttutor`, "
...
def start(home: str | Path | None = None, *, dev: bool = False) -> None:
    ...
    os.environ[SMARTTUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env[SMARTTUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env["SMARTTUTOR_API_BASE_URL"] = backend_url
    common_env["SMARTTUTOR_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"
    # Anchor for smarttutor.runtime.memory_probe: the backend and the frontend
    # are siblings under this process, so only the supervisor's pid identifies
    # the tree that is "SmartTutor". Without it the probe can only measure the
    # backend itself.
    common_env[SUPERVISOR_PID_ENV] = str(os.getpid())
    _apply_single_user_allocator_env(common_env)
    if frontend.kind == "source-production":
        common_env["SMARTTUTOR_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR

    backend_cmd = [
        sys.executable,
        "-m",
        "uvicorn",
        "smarttutor.api.main:app",
        "--host",
        "0.0.0.0",
        "--port",
        str(backend_port),
        ...
        # middleware (smarttutor/api/main.py) surfaces only non-200s, so routine
