"""Resident-memory snapshot of the running SmartTutor process tree.

The counterpart to :mod:`smarttutor.runtime.memory_reclaim`: that module *acts*
...
Two things make "how much memory does SmartTutor use" more than one ``getrusage``
call:

* The backend is only one of the processes. ``smarttutor.runtime.launcher``
  ...
  ``uvicorn smarttutor.api.main:app`` run would climb into the developer's shell
...
# find the root of the SmartTutor process tree it belongs to.
SUPERVISOR_PID_ENV = "SMARTTUTOR_SUPERVISOR_PID"
...
class ProcessMemory:
    """One process in the SmartTutor tree."""
...
def _classify(pid: int, name: str, cmdline: str, root_pid: int | None = None) -> str:
    ...
    the supervisor runs as a bare ``smarttutor`` console script with nothing in
    ...
    if "next" in lowered or "node" in lowered:
        return "web"
    if "uvicorn" in lowered or "smarttutor.api" in lowered:
        return "backend"
...
def _scan_proc() -> tuple[list[ProcessMemory], bool]:
    """psutil-free fallback for Linux — the platform SmartTutor ships in Docker on."""
