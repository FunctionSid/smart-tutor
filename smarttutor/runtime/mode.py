...
Controls whether SmartTutor is running as a CLI application or an API server.
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("SMARTTUTOR_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    ...
    global _current_mode
    _current_mode = mode
    os.environ["SMARTTUTOR_MODE"] = mode.value
