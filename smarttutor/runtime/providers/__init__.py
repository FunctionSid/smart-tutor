...
An *external provider* is something that contributes tools SmartTutor did not
...
``load_tools`` (see :mod:`smarttutor.runtime.registry.deferred_tools`). What
they do *not* share is authorisation, so that stays per-kind and explicit in
:mod:`~smarttutor.runtime.providers.authorize`.
...
upwards. Import it directly (``from smarttutor.runtime.providers.view import
...
from smarttutor.runtime.providers.text import (
