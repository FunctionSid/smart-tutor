"""
Authentication service for SmartTutor.
...
Quick setup (single user via data/user/settings/auth.json):
    ...
    3. Generate a password hash:
           python -c "from smarttutor.services.auth import hash_password; print(hash_password('yourpassword'))"
...
from smarttutor.services.config import load_auth_settings, load_integrations_settings
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from smarttutor.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(hashed: str, role: str = "user", created_at: str = "") -> dict[str, Any]:
    ...
    from smarttutor.multi_user.identity import new_user_id
...
def _load_users() -> dict[str, dict]:
    ...
    """
    from smarttutor.multi_user.identity import load_users
...
def add_user(username: str, plain_password: str, role: str = "user") -> None:
    ...
    """
    from smarttutor.multi_user.identity import save_user
...
def list_users() -> list[dict]:
    ...
    from smarttutor.multi_user.identity import list_user_info
...
def delete_user(username: str) -> bool:
    ...
    """
    from smarttutor.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from smarttutor.multi_user.identity import set_role as _set_role
...
def set_avatar(username: str, avatar: str) -> bool:
    ...
    """
    from smarttutor.multi_user.identity import set_avatar as _set_avatar
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from smarttutor.services.pocketbase_client import validate_pb_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@smarttutor.local to match the email used at registration.
    """
    try:
