...
entry's provenance (see :class:`~smarttutor.services.cli_apps.models.AppTrust`).

What this module does *not* do:

* **Run the registry's install command.** It runs an argv assembled from an
  :class:`~smarttutor.services.cli_apps.models.InstallPlan`. See ``models``.
...
INSTALL_TIMEOUT_S = int(os.environ.get("SMARTTUTOR_CLI_APP_INSTALL_TIMEOUT_S", "900"))
...
def _steps(entry: CliAppEntry, env_dir) -> list[tuple[list[str], dict[str, str]]]:
    ...
    if plan.runtime is AppRuntime.PYTHON:
        base = _clean_env()
        python_bin = Path(env_dir) / ("Scripts" if os.name == "nt" else "bin") / (
...
def _pin_of(entry: CliAppEntry) -> str:
    ...
    if entry.install.kind is InstallKind.PINNED_HARNESS:
        from smarttutor.services.cli_apps.catalog import catalog_pin
