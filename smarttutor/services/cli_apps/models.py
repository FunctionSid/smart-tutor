    ...
    if not text:
        return InstallPlan(
            ...
            reason=(
                ...
                "there is nothing for SmartTutor to install."
            ),
        )
    ...
    if any(token in text for token in ("&&", "||", "|", ">", ";", "$(", "`")):
        return InstallPlan(
            ...
            reason=(
                ...
                "single package install. SmartTutor builds every install command "
    ...
    if len(requirements) != 1:
        return InstallPlan(
            ...
            reason=(
                ...
                "which SmartTutor does not model as a single app."
            ),
        )
...
def _unsupported_manager(manager: str) -> InstallPlan:
    return InstallPlan(
        ...
        trust=AppTrust.THIRD_PARTY,
        reason=(
            f"Installs with {manager}, which the SmartTutor runtime image does not "
