...
from smarttutor.services.cli_apps.models import ENTRY_POINT_RE, AppRuntime
...
def cli_apps_root() -> Path:
    ...
    from smarttutor.multi_user.paths import ADMIN_WORKSPACE_ROOT
...
def bin_dir(app_id: str, runtime: AppRuntime) -> Path:
    if runtime is AppRuntime.PYTHON and os.name == "nt":
...
def executable_path(app_id: str, runtime: AppRuntime, entry_point: str) -> Path:
    ...
    path = bin_dir(app_id, runtime) / entry_point
    if os.name == "nt" and runtime is AppRuntime.PYTHON:
        for candidate in (
            path.with_suffix(".exe"),
            path.with_suffix(".cmd"),
            path,
            runtime_dir(app_id, runtime) / "bin" / entry_point,
        ):
            if candidate.exists():
                return candidate
    return path
