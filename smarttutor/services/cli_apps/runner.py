...
exec path in SmartTutor whose arguments are model-authored *and* whose program is
...
from smarttutor.services.cli_apps.paths import abi_stamp, bin_dir, executable_path, runtime_dir
from smarttutor.services.cli_apps.state import InstalledApp
from smarttutor.services.sandbox.spec import ExecRequest, ExecResult, Mount, ResourceLimits
...
async def run_app(
    ...
    *,
    ...
    from smarttutor.services.sandbox import get_sandbox_service
...
def _path_for(app: InstalledApp) -> str:
    ...
    for candidate in (*_split_host_path(os.environ.get("PATH", "")), "/usr/local/bin", "/usr/bin", "/bin"):
        if candidate and candidate not in parts:
            parts.append(candidate)
    return os.pathsep.join(parts)


def _split_host_path(value: str) -> list[str]:
    if not value:
        return []
    parts = value.split(os.pathsep)
    if os.pathsep == ";" and len(parts) == 1 and re.search(r"(^|:)/", value):
        parts = value.split(":")
