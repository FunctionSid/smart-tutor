...
from smarttutor.services.config.runtime_settings import load_system_settings
...
def _managed_profile(
    ...
    *,
    ...
    profile = {
        ...
        # other users through grants (see smarttutor/multi_user/model_access.py).
        "owner_bound": True,
...
def _codex_user_root() -> Path:
    ...
    """
    from smarttutor.multi_user.paths import get_owner_path_service
...
def _codex_secrets_root() -> Path:
    ...
    """
    from smarttutor.multi_user.paths import get_owner_secrets_dir
...
def _relocate_legacy_store(user_root: Path, secrets_root: Path) -> bool:
    ...
    :func:`~smarttutor.multi_user.paths.migrate_legacy_multi_user_tree`.
...
def _owner_model_catalog_service() -> ModelCatalogService:
    ...
    places (#781).
    """
    from smarttutor.multi_user.personal_models import owner_catalog_service
...
async def deliver_codex_oauth_callback(
    ...
    far end of the user's tunnel — not on the SmartTutor Web origin, so the
