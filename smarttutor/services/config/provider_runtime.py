"""Nanobot-style normalized runtime configuration for SmartTutor."""
...
from smarttutor.services.imagegen.config import ImagegenConfig
from smarttutor.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
from smarttutor.services.provider_registry import (
...
from smarttutor.services.videogen.config import VideogenConfig
from smarttutor.services.voice.config import (
    AUTH_API_KEY_HEADER,
    AUTH_BEARER,
    STT_BASE64_JSON,
    STT_MULTIPART,
...
class SearchProviderSpec:
    ...
    its own answer or needs :mod:`smarttutor.services.search.consolidation` to
    ...
    The provider registry (``smarttutor/services/search/providers``), the
    Settings dropdown (``smarttutor/api/routers/settings.py``) and the web
...
TTS_PROVIDERS: dict[str, VoiceProviderSpec] = {
    ...
    "edge_tts": VoiceProviderSpec(
        label="Edge TTS / Global",
        default_api_base="edge-tts",
        adapter="global_edge_tts",
        default_model="edge-tts",
        default_voice="en-US-AriaNeural",
        is_local=True,
    ),
    "windows_speech": VoiceProviderSpec(
        label="Windows System.Speech",
        default_api_base="powershell",
        adapter="windows_system_speech",
    ...
    "piper": VoiceProviderSpec(
        label="Piper / Global",
        default_api_base="piper",
        adapter="global_piper",
        default_model="",
        default_voice="",
        is_local=True,
    ),
    "pyttsx3": VoiceProviderSpec(
        label="pyttsx3 / Global",
        default_api_base="python",
        adapter="global_pyttsx3",
        default_model="pyttsx3",
        default_voice="Microsoft Zira Desktop",
        is_local=True,
    ),
    "gtts": VoiceProviderSpec(
        label="gTTS / Global",
        default_api_base="python",
        adapter="global_gtts",
        default_model="en",
...
STT_PROVIDERS: dict[str, VoiceProviderSpec] = {
    ...
    "faster_whisper": VoiceProviderSpec(
        label="faster-whisper / Global",
        default_api_base="python",
...
VOICE_PROVIDER_ALIASES = {
    ...
    "lmstudio": "vllm",
    "edge-tts": "edge_tts",
    "global_edge_tts": "edge_tts",
    "system_speech": "windows_speech",
    "windows_system_speech": "windows_speech",
    "global_faster_whisper": "faster_whisper",
    "faster-whisper": "faster_whisper",
    "global_pyttsx3": "pyttsx3",
    "global_gtts": "gtts",
}
...
def _with_personal_llm_profiles(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    the shared one (see :mod:`smarttutor.multi_user.personal_models`), so
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    if spec.is_local and api_key.strip().lower() in {"", "local", "ollama", "none"}:
