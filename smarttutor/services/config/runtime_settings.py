...
from smarttutor.services.file_io import atomic_write_json as _atomic_write_json
...
# Document parsing settings. The parse layer (smarttutor/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with SmartTutor's.
    "local_cli_path": "",
...
# SmartTutor's VLM to describe images.
...
# SmartTutor's legacy text extractors for PDF / Office / text-like files.
...
IGNORE_PROCESS_OVERRIDES_ENV = "SMARTTUTOR_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "SMARTTUTOR_SANDBOX_ALLOW_SUBPROCESS": _bool_env(system["sandbox_allow_subprocess"]),
            ...
            # rewrites /api/* and /ws/* to SMARTTUTOR_API_BASE_URL and uses
            # SMARTTUTOR_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. SMARTTUTOR_API_BASE_URL is
            ...
            "SMARTTUTOR_AUTH_ENABLED": _bool_env(auth["enabled"]),
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("SMARTTUTOR_SANDBOX_ALLOW_SUBPROCESS"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (SMARTTUTOR_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("SMARTTUTOR_RAG_RETRIEVAL_PROFILE")
