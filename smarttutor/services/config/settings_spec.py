"""Machine-readable description of what a SmartTutor install can be told to be.
...
(:mod:`smarttutor.capabilities.setup.tools`) completely generic: they walk this
...
``scope``
    ...
    row is one that :class:`~smarttutor.services.path_service.PathService`
    already resolves per user, a global row is one read through
    :func:`~smarttutor.services.config.loader.get_runtime_settings_dir`.
...
``probe``
    ...
    :func:`~smarttutor.capabilities.setup.apply.apply_setting` refuses to commit
...
Imports of the rest of SmartTutor are deferred into the row callables on
purpose: this module is imported from ``smarttutor.capabilities``, and a
...
def _write_ui(key: str) -> Callable[[str], None]:
    def _write(value: str) -> None:
        from smarttutor.services.settings.interface_settings import set_ui_setting
...
def _interface_specs() -> list[SettingSpec]:
    ...
    return [
        SettingSpec(
            ...
            summary="Language of the SmartTutor UI (menus, buttons, settings).",
            read=language_read,
...
def _parsing_engine_choices() -> tuple[SettingChoice, ...]:
    ...
    """
    from smarttutor.services.parsing.engines._install import installable_engines
    from smarttutor.services.parsing.engines.factory import list_engines
...
def _validate_parsing_engine(value: str) -> str | None:
    from smarttutor.services.parsing.engines.factory import is_engine_available
...
def _catalog_profile_choices(service_name: str) -> Callable[[], tuple[SettingChoice, ...]]:
    def _choices() -> tuple[SettingChoice, ...]:
        from smarttutor.services.config.provider_runtime import SEARCH_PROVIDERS
...
def _clear_runtime_caches() -> None:
    ...
    """
    try:
        from smarttutor.services.llm import clear_llm_config_cache
...
async def _probe_llm(value: str) -> ProbeResult:
    ...
    from smarttutor.services.config.provider_runtime import resolve_llm_runtime_config
    from smarttutor.services.llm import complete as llm_complete
    from smarttutor.services.llm import get_token_limit_kwargs
...
async def _probe_embedding(value: str) -> ProbeResult:
    ...
    from smarttutor.services.config.provider_runtime import resolve_embedding_runtime_config
    from smarttutor.services.embedding.client import EmbeddingClient
    from smarttutor.services.embedding.config import EmbeddingConfig
...
def setting_specs() -> dict[str, SettingSpec]:
    ...
    ``smarttutor.capabilities`` without closing an import cycle.
    """
