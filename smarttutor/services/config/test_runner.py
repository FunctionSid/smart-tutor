...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from smarttutor.services.llm import clear_llm_config_cache, get_token_limit_kwargs
    ...
    async def _test_embedding(
        ...
        probe_texts = [
            "SmartTutor embedding smoke test",
            "SmartTutor retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from smarttutor.services.search import web_search
        ...
        run.emit("info", "Running search query: SmartTutor configuration health check")
        result = web_search("SmartTutor configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from smarttutor.services.config.provider_runtime import resolve_tts_runtime_config
        from smarttutor.services.voice import synthesize_speech
        ...
        sample = "SmartTutor voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from smarttutor.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from smarttutor.services.config.provider_runtime import resolve_imagegen_runtime_config
        from smarttutor.services.imagegen import generate_image
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from smarttutor.services.config.provider_runtime import resolve_videogen_runtime_config
        from smarttutor.services.videogen import probe_video
