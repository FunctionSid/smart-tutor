...
from smarttutor.services.cron.service import CronJob
...
async def _maybe_send_desktop_notification(job: CronJob, text: str) -> None:
    ...
    SmartTutor still persists/delivers through its normal chat or partner
    ...
    title = f"SmartTutor: {job.name or 'Reminder'}"
...
async def _execute_chat_job(job: CronJob) -> tuple[str, str | None]:
    ...
    from smarttutor.multi_user.paths import local_admin_user, scope_for_user, user_context
    from smarttutor.runtime.orchestrator import ChatOrchestrator
    from smarttutor.services.session import get_sqlite_session_store
