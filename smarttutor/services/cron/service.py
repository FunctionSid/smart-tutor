...
SmartTutor runs one server process, so a single in-process scheduler owns
the store.
...
executor lives in :mod:`smarttutor.services.cron.executor`.
"""
...
def get_cron_service() -> CronService:
    ...
    if _service is None:
        ...
        from smarttutor.services.cron.executor import execute_job
