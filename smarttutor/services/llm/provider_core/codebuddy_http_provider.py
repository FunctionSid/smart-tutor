...
class CodeBuddyHTTPProvider(OpenAICompatProvider):
    ...
    async def _chat_impl(
        ...
        **kwargs: Any,
        ...
        kwargs.pop("smarttutor_session_id", None)
...
def build_codebuddy_provider(
    ...
    ``SMARTTUTOR_CODEBUDDY_BACKEND`` (``http`` / ``sdk``) forces one transport;
    the SDK is still needed for CLI-driven agent features such as sandboxed
    shell tools.
    """
    preference = (os.environ.get("SMARTTUTOR_CODEBUDDY_BACKEND") or "").strip().lower()
    ...
    from smarttutor.services.llm.provider_core.codebuddy_provider import CodeBuddyProvider
