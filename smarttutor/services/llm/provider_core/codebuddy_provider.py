...
from smarttutor.services.llm.provider_core.base import (
    LLMProvider,
    LLMResponse,
    ToolCallRequest,
)
...
class _CodeBuddySession:
    ...
    ``connect()`` and must exit them from the same task. SmartTutor chat turns
...
class CodeBuddyProvider(LLMProvider):
    ...
    endpoint. This adapter keeps one stateful CLI process per SmartTutor session
    and exposes SmartTutor's tool schemas through an in-process SDK MCP server.
    Tool execution remains owned by SmartTutor's existing dispatcher.
    """
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        session_id = str(kwargs.pop("smarttutor_session_id", "") or "").strip()
...
def _build_options(
    ...
    raise RuntimeError(
        ...
        '(pip install "smarttutor[codebuddy]").'
    )
                ...
                "content": [
                    {
                        "type": "text",
                        "text": (
                            "SmartTutor accepted this tool call and will provide its result next."
                        ),
                    }
                ]
            }

        pending_result.__name__ = f"smarttutor_{name}"
...
def _reasoning_options(reasoning_effort: str | None) -> dict[str, Any]:
    """Map SmartTutor reasoning levels to CodeBuddy SDK thinking controls.
...
def _friendly_error(exc: Exception) -> str:
    ...
    if "Authentication required" in message:
        return (
            ...
            "`smarttutor provider login codebuddy`, run `codebuddy` and enter `/login`, "
