"""OpenAI Codex Responses provider backed by SmartTutor's private OAuth service."""
...
from smarttutor.services.codex_auth import CodexAuthError, get_codex_oauth_service
from smarttutor.services.codex_auth.constants import CODEX_DEFAULT_MODEL_ID, CODEX_RESPONSES_URL
from smarttutor.services.codex_auth.contracts import CodexToken
from smarttutor.services.llm.exceptions import LLMProviderTransportError
from smarttutor.services.llm.openai_http_client import disable_ssl_verify_enabled
from smarttutor.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
from smarttutor.services.llm.provider_core.openai_responses import (
    consume_sse,
    convert_messages,
    convert_tools,
)
...
def _build_headers(account_id: str, token: str) -> dict[str, str]:
    if not token:
        raise RuntimeError(
            "OpenAI Codex is not logged in. Run `smarttutor provider login openai-codex`."
        )
    headers = {
        "Authorization": f"Bearer {token}",
        "OpenAI-Beta": "responses=experimental",
        "originator": DEFAULT_ORIGINATOR,
        "User-Agent": "SmartTutor (python)",
