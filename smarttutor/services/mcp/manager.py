...
SmartTutor's chat runs as per-turn tasks inside one event loop, while MCP
...
class MCPConnectionManager:
    ...
    async def ensure_scope(self, owner: str) -> list[MCPToolAdapter]:
        ...
        from smarttutor.services.mcp.user_config import load_user_mcp_config
    ...
    def _signature(cls, cfg: MCPServerConfig, owner: str) -> str:
        ...
        :mod:`smarttutor.services.mcp.secrets`). Rotating a credential therefore
    ...
    def _materialize(cfg: MCPServerConfig, owner: str) -> MCPServerConfig:
        ...
        one transport open. See :mod:`smarttutor.services.mcp.secrets` for why the
        stored config holds references instead.
        """
        from smarttutor.services.mcp.secrets import resolve_references, resolve_url_references
    ...
    async def _open_transport(
        ...
        *,
        ...
        if self_service:
            ...
            from smarttutor.services.mcp.network import validate_mcp_url_async
        ...
        oauth_auth = None
        if cfg.auth == "oauth":
            from smarttutor.services.mcp.oauth import build_auth, oauth_redirect_uri
...
def _needs_authorization(exc: BaseException) -> bool:
    ...
    """
    from smarttutor.services.mcp.oauth import AuthorizationRequired
