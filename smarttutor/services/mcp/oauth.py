...
sibling module :mod:`smarttutor.services.mcp.secrets` holds static credentials in
...
#: How SmartTutor introduces itself on a consent screen.
CLIENT_NAME = "SmartTutor"
CLIENT_URI = "https://smart-tutor.local"
...
def _store_path(owner_id: str, server: str) -> Path:
    from smarttutor.multi_user.paths import owner_secrets_dir
...
_PUBLIC_URL_ENV = "SMARTTUTOR_PUBLIC_URL"
...
def oauth_redirect_uri(origin: str = "") -> str:
    ...
    *origin* is the origin the person is actually browsing, which the route that
    ...
    deployment work with no configuration. ``SMARTTUTOR_PUBLIC_URL`` overrides it
...
def client_metadata(redirect_uri: str) -> "Any":
    """How SmartTutor registers itself with an authorization server."""
