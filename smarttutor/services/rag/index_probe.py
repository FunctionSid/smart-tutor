...
SmartTutor owns KB lifecycle/status, but each RAG provider owns the shape of its
...
def _inspect_graphrag(storage_dir: Path) -> ProviderIndexProbe:
    from smarttutor.services.rag.pipelines.graphrag import storage
...
def _inspect_lightrag(storage_dir: Path) -> ProviderIndexProbe:
    from smarttutor.services.rag.pipelines.lightrag import storage
