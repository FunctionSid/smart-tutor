"""Bridge SmartTutor's runtime config into a GraphRAG ``settings.yaml``.
...
SmartTutor's already-resolved LLM + embedding runtime config and let
...
* Completion calls use SmartTutor's registered GraphRAG compatibility adapter,
...
from smarttutor.services.config.embedding_endpoint import canonical_embedding_provider_name
from smarttutor.services.embedding.request_options import should_send_embedding_dimensions
...
# appends the ``/embeddings`` operation path to ``api_base``. SmartTutor stores a
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when SmartTutor's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'smarttutor[graphrag]'``. Until it is installed the
...
def graphrag_embedding_api_base(binding: str | None, endpoint: str | None) -> str:
    """Translate a SmartTutor embedding endpoint into GraphRAG ``api_base``.

    SmartTutor's public embedding contract stores and calls the complete
    ...
    Args:
        binding: Active SmartTutor embedding binding.
...
def ensure_graphrag_embedding_transport(
    ...
    # Gemini can use either SmartTutor's native ``batchEmbedContents`` adapter
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from smarttutor.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from SmartTutor config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from SmartTutor's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from smarttutor.services.config import resolve_llm_runtime_config
    ...
    return {
        ...
        # Plain-text input: SmartTutor's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; SmartTutor must stamp the
