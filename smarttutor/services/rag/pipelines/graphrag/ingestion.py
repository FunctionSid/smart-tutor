...
(its optional ``markitdown`` reader is just a converter). So SmartTutor owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse SmartTutor's existing extraction primitives here so multimodal/parsing
stays a SmartTutor-side concern (matching how the LlamaIndex pipeline already
