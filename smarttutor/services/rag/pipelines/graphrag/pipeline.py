...
SmartTutor first (see ``ingestion``) so GraphRAG only ever sees ``.txt`` input.
...
class GraphRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not gr_config.is_graphrag_available():
            raise gr_config.GraphRagNotAvailableError(
                ...
                "`pip install 'smarttutor[graphrag]'` to use GraphRAG knowledge bases."
            )
    ...
    async def _preflight_settings(self, settings: dict[str, Any]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="smarttutor-graphrag-preflight-") as temp_dir:
...
def _context_to_sources(context_data: dict[str, Any]) -> list[dict[str, Any]]:
    """Map GraphRAG context records into SmartTutor's source-citation shape."""
