"""Bridge SmartTutor's runtime config into LightRAG / RAG-Anything.
...
builds the three adapters LightRAG needs from SmartTutor's already-resolved LLM /
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap SmartTutor's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses SmartTutor's embedding client, wrapped in LightRAG's
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when SmartTutor's LLM / embedding config can't back LightRAG."""
...
def _retry_classification(exc: Exception) -> tuple[bool, str]:
    ...
    from smarttutor.services.llm.request_compat import is_transient_transport_error
...
def _retry_delay_seconds(exc: Exception, scheduled_delay: float) -> float:
    ...
    from smarttutor.services.llm.error_mapping import retry_after_seconds
...
def is_lightrag_available() -> bool:
    ...
    Opt-in extra: ``pip install 'smarttutor[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    """
    try:
        from smarttutor.services.config import load_lightrag_settings
...
def build_llm_model_func(*, io_bridge: OwnerLoopBridge | None = None):
    """Wrap SmartTutor's unified LLM callable for LightRAG.
...
def build_vision_model_func(*, io_bridge: OwnerLoopBridge | None = None):
    """Wrap SmartTutor's vision-capable callable for RAG-Anything's image step."""
    from smarttutor.services.llm import get_llm_client
...
def build_embedding_func(*, io_bridge: OwnerLoopBridge | None = None):
    """Wrap SmartTutor's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from smarttutor.services.embedding import get_embedding_client, get_embedding_config
