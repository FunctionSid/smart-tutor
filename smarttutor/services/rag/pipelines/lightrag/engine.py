...
fix. All imports are lazy so SmartTutor runs fine without the optional dependency
installed.

A RAG-Anything instance is built from SmartTutor's LLM/vision/embedding adapters
...
def build_rag(working_dir: Path, *, io_bridge: OwnerLoopBridge | None = None) -> Any:
    ...
    # SmartTutor always feeds RAG-Anything a pre-parsed ``content_list`` (the
    # parse layer runs upstream via SmartTutor's own ParseService), so
