...
Documents are turned into a MinerU-style ``content_list`` by SmartTutor's shared
parse layer (``smarttutor/services/parsing``) — the same bridge the question
...
from smarttutor.runtime.home import get_runtime_data_root
from smarttutor.services.rag.index_versioning import (
    resolve_storage_dir_for_read,
    resolve_storage_dir_for_rebuild,
)
from smarttutor.services.rag.kb_paths import resolve_kb_dir
...
class LightRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'smarttutor[rag-lightrag]'` to use LightRAG knowledge bases."
            )
    ...
    async def _ingest(
        ...
        *,
        ...
        """
        from smarttutor.services.parsing import ParserError, get_parse_service
    ...
    async def _run_indexing(
        ...
        SmartTutor-owned network calls and progress callbacks cross back through
