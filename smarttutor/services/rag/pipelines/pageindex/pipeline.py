...
class PageIndexPipeline:
    """Manage one PageIndex provider while preserving SmartTutor's RAG contract."""
    ...
    def _processing_mode(self, kb_name: str) -> str | None:
        if self.provider != storage.OSS_PROVIDER:
            return None
        try:
            from smarttutor.services.config.knowledge_base_config import KnowledgeBaseConfigService
