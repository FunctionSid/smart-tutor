...
class RAGService:
    ...
    SmartTutor's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from SmartTutor's binding."""
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from smarttutor.logging import ProcessLogEvent, capture_process_logs
        from smarttutor.logging.formatters import ContextFilter
        ...
        def should_skip_noisy_retrieve_log(event: ProcessLogEvent) -> bool:
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("smarttutor.services.embedding.")
                and (
