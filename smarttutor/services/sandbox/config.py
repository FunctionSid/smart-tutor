...
* ``SMARTTUTOR_SANDBOX_RUNNER_URL`` set ⇒ runner sidecar (Docker deployment);
* else on Linux with a functional ``bwrap`` ⇒ bwrap (bare-metal);
* else, only when ``SMARTTUTOR_SANDBOX_ALLOW_SUBPROCESS=1`` ⇒ restricted
...
:mod:`smarttutor.tools.exec_tool`). Per-user quotas live in
:mod:`smarttutor.services.sandbox.quota`.
"""
...
from smarttutor.services.sandbox.backends import (
    BwrapBackend,
    RestrictedSubprocessBackend,
    RunnerSidecarBackend,
    SandboxBackend,
)
from smarttutor.services.sandbox.spec import ResourceLimits

RUNNER_URL_ENV = "SMARTTUTOR_SANDBOX_RUNNER_URL"
ALLOW_SUBPROCESS_ENV = "SMARTTUTOR_SANDBOX_ALLOW_SUBPROCESS"

# Per-user execution quotas (see quota.py). Conservative defaults; override
# via the matching env vars.
MAX_CONCURRENT_ENV = "SMARTTUTOR_SANDBOX_MAX_CONCURRENT"
MAX_PER_MINUTE_ENV = "SMARTTUTOR_SANDBOX_MAX_PER_MINUTE"
...
def build_backend(settings: SandboxSettings) -> SandboxBackend | None:
    ...
    backend's ``health()`` — see :mod:`smarttutor.services.sandbox.service`.
    """
    import sys
