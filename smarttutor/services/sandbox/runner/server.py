...
(:class:`smarttutor.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`smarttutor.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``SMARTTUTOR_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "SMARTTUTOR_RUNNER_ALLOWED_WORKDIRS",
