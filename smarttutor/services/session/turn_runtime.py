...
from smarttutor.core.stream import StreamEvent, StreamEventType
from smarttutor.services.llm.utils import clean_thinking_tags
from smarttutor.services.path_service import get_path_service
from smarttutor.services.session.artifact_attachments import (
    artifact_attachments,
    fill_preview_text,
)
from smarttutor.services.session.protocol import SessionStoreProtocol

if TYPE_CHECKING:
    from smarttutor.learning.storage import MasteryPathLease
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from smarttutor.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from smarttutor.services.session import get_session_store
    ...
    async def _acquire_mastery_path_lease(
        ...
        *,
        ...
        from smarttutor.learning.storage import LearningStore, PathLeaseConflictError
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from smarttutor.runtime.request_contracts import validate_capability_config
        ...
        if capability == "mastery_path":
            from smarttutor.learning.identity import resolve_mastery_path_binding
        ...
        if llm_selection:
            try:
                from smarttutor.multi_user.model_access import apply_allowed_llm_selection
            ...
            from smarttutor.multi_user.model_access import (
        ...
        if llm_selection:
            from smarttutor.multi_user.personal_models import merge_personal_llm_profiles
            from smarttutor.services.config import get_model_catalog_service
            from smarttutor.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from smarttutor.api.routers.settings import get_enabled_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from smarttutor.agents.notebook import NotebookAnalysisAgent
            from smarttutor.book.context import build_book_context
            from smarttutor.core.context import Attachment, UnifiedContext
            from smarttutor.runtime.orchestrator import ChatOrchestrator
            from smarttutor.services.memory import get_memory_store
            ...
            from smarttutor.services.session.context_builder import ContextBuilder
            from smarttutor.services.skill import get_skill_service
            ...
            from smarttutor.services.storage import get_attachment_store
            ...
            from smarttutor.utils.document_extractor import extract_documents_from_records
            ...
            from smarttutor.multi_user.skill_access import assigned_skill_ids
            from smarttutor.services.persona import PersonaService, get_persona_service
            from smarttutor.services.skill.service import SkillService, render_skills_manifest
            ...
            if is_chat_capability:
                from smarttutor.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
            ...
            from smarttutor.runtime.memory_reclaim import schedule_memory_reclaim
    ...
    async def _maybe_generate_session_title(
        ...
        *,
        ...
        title = ""
        try:
            from smarttutor.services.llm import stream as llm_stream
