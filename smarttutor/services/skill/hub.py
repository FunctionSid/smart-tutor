...
A hub skill is the same artefact SmartTutor already speaks natively — a
...
# SmartTutor ships pointing at EduHub, its native open skill registry: a bare
# ``install <slug>`` with no ``<hub>:`` prefix resolves here.
DEFAULT_HUB = "eduhub"
_CLAWHUB_BASE_URL = "https://clawhub.ai/api/v1"
_EDUHUB_BASE_URL = "https://eduhub.smarttutor.info/api/v1"
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="smarttutor-skill-"))
        try:
    ...
    def catalog(
        ...
        SmartTutor-native browser renders without a second round-trip.
        """
        if query.strip():
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        try:
            try:
                ...
                raise HubError(f"{self.name}: fetch command not found: {argv[0]}") from exc
