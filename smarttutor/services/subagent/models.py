...
reasoning effort SmartTutor drives each backend with. Those lists change over
...
from smarttutor.services.subagent.process import probe_version, resolve_cli_command
from smarttutor.services.subagent.registry import get_backend
...
async def _claude_options() -> BackendOptions:
    from smarttutor.services.subagent.claude_models import load_cached_claude_models
...
async def sync_backend_options(kind: str) -> BackendOptions:
    ...
    """
    if kind == "claude_code":
        from smarttutor.services.subagent.claude_models import sync_claude_models
