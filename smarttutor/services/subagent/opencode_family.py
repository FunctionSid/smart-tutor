...
HTTP server (managed by :mod:`smarttutor.services.subagent.opencode_server`),
...
from smarttutor.services.subagent.opencode_server import acquire_server
from smarttutor.services.subagent.process import probe_version
...
class OpencodeFamilyBackend(SubagentBackend):
    ...
    async def _create_session(self, client: httpx.AsyncClient) -> str:
        response = await client.post("/session", json={"title": "SmartTutor consult"})
        response.raise_for_status()
        data = response.json()
