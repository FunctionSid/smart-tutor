...
*partner session key*. The first consult of a SmartTutor chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`smarttutor.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same SmartTutor chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per SmartTutor chat, titled from the
...
from smarttutor.services.subagent.base import OnEvent, SubagentBackend
from smarttutor.services.subagent.config import BackendConfig
from smarttutor.services.subagent.types import (
    EVENT_ERROR,
    EVENT_REASONING,
    EVENT_TEXT,
    EVENT_TOOL,
...
if TYPE_CHECKING:  # avoid importing the core/partner packages at module load
    from smarttutor.core.stream import StreamEvent
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        from smarttutor.multi_user.partner_access import assert_partner_allowed
        ...
        # a SmartTutor chat → mint a stable, colon-free key; the registry threads
