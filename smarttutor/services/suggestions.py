...
def _render_l3(cap: int) -> str:
    ...
    Reads through :class:`~smarttutor.services.memory.store.MemoryStore`'s
...
def _collect_material(trace_count: int) -> _Material:
    ...
    nothing about that.
    """
    from smarttutor.services.memory import recall
...
async def _generate(language: str, material: _Material) -> SuggestionSet:
    ...
    try:
        from smarttutor.services.llm import complete
