...
from smarttutor.capabilities.ima import IMA_TOOL_TYPES
from smarttutor.capabilities.marginnote4 import MARGINNOTE_TOOL_TYPES
from smarttutor.capabilities.mastery import MASTERY_TOOL_TYPES
from smarttutor.capabilities.obsidian import OBSIDIAN_TOOL_TYPES
from smarttutor.capabilities.reading import READING_TOOL_TYPES
from smarttutor.capabilities.setup import SETUP_TOOL_TYPES
from smarttutor.capabilities.solve import SOLVE_TOOL_TYPES
from smarttutor.capabilities.subagent import SUBAGENT_TOOL_TYPES
...
from smarttutor.tools.prompting import load_prompt_hints
from smarttutor.tools.question_bank import ACTIONS as QB_ACTIONS
from smarttutor.tools.question_bank import FILTERS as QB_FILTERS
...
class BrainstormTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.brainstorm import brainstorm
...
class KbFilesTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.knowledge.manifest import render_manifest_report
...
class WebSearchTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.web_search import web_search
...
class CodeExecutionTool(_PromptHintsMixin, BaseTool):
    ...
    it through :mod:`smarttutor.services.sandbox`. No second LLM call, and the
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from smarttutor.services.sandbox import (
            ExecRequest,
            Mount,
            ResourceLimits,
            get_sandbox_service,
        )
        from smarttutor.services.sandbox.artifacts import (
...
class ReasonTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.reason import reason
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.agents.vision_solver.vision_solver_agent import VisionSolverAgent
        from smarttutor.services.llm.config import get_llm_config
...
class WriteMemoryTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.services.memory import get_memory_store
        from smarttutor.services.memory.trace import TraceEvent
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``smarttutor.tools.web_fetch`` so this wrapper stays free of network
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.web_fetch import (
            DEFAULT_MAX_CHARS,
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.list_notebook import list_notebooks_or_records
...
class QuestionBankTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.question_bank import run_question_bank
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.github_query import run_github_query
...
class AskUserTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.ask_user import build_ask_user_payload
...
class ReadSkillTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from smarttutor.services.skill.service import (
...
class CronTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.tools.cron_tool import run_cron_action
