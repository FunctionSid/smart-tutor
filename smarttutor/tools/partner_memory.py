...
def _concat_l3() -> str:
    ...
    """
    from smarttutor.services.memory import get_memory_store, paths
...
def _resolve_partner_id() -> str | None:
    ...
    from smarttutor.multi_user.context import get_current_user_or_none
    from smarttutor.services.partners.scope import PARTNER_USER_PREFIX
...
class PartnerReadTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.multi_user.paths import get_admin_path_service, get_current_path_service
        from smarttutor.services.memory import memory_path_service_override
        from smarttutor.services.partners.interaction import get_partner_turn_context
...
class PartnerMemorizeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from smarttutor.multi_user.paths import get_current_path_service
        from smarttutor.services.memory import get_memory_store, memory_path_service_override
