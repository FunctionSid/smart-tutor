...
# lives at module scope so a single ``smarttutor chat`` session shares one
# ring across turns; ``smarttutor run`` doesn't read it (single-shot mode),
...
async def stream_turn_as_json(*, app: SmartTutorApp, turn_id: str) -> None:
...
async def render_turn_stream(*, app: SmartTutorApp, turn_id: str) -> str:
...
class TurnStreamRenderer:
    ...
    async def _on_wait_for_input(self, item: dict[str, Any]) -> None:
        ...
        from smarttutor.core.stream_bus import get_bus
