...
``smarttutor init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`smarttutor_cli.init_wizard`. This module is
...
from smarttutor.runtime.home import SMARTTUTOR_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new SMARTTUTOR_HOME takes effect.

    ``smarttutor init`` may pass ``--home`` to target a different workspace; the
...
def _embedding_step(
    ...
    from smarttutor.services.config.embedding_endpoint import (
        EMBEDDING_PROVIDER_LABELS,
        normalize_embedding_endpoint_for_display,
        redact_embedding_endpoint_for_display,
    )
    from smarttutor.services.config.provider_runtime import EMBEDDING_PROVIDERS
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    from smarttutor.services.config import get_model_catalog_service, get_runtime_settings_service
