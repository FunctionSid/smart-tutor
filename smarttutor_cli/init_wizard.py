"""Interactive setup helpers used by ``smarttutor init``.
...
from smarttutor.services.llm.config import get_token_limit_kwargs
from smarttutor.services.provider_registry import PROVIDERS, ProviderSpec, find_by_name
...
# ``smarttutor.services.config.provider_runtime``. Adding a new featured entry
...
# ``smarttutor.services.config.provider_runtime``. Each entry below describes
...
def _all_providers_except(featured: set[str]) -> list[ProviderSpec]:
    ...
    return [
        ...
        if spec.name not in featured and not spec.is_oauth  # OAuth flows use `smarttutor login`
    ]
