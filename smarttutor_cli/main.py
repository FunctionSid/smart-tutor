"""CLI entry point for the standalone ``smarttutor-cli`` package."""
...
from smarttutor.runtime.mode import RunMode, set_mode
...
app = typer.Typer(
    name="smarttutor",
    help="Smart Tutor CLI - agent-first interface for capabilities, tools, and knowledge.",
...
skill_app = typer.Typer(help="Manage skills and install from hubs.")
...
app.add_typer(skill_app, name="skills")  # alias: `smarttutor skills …`
...
def run_capability(
    ...
    from smarttutor.app import SmartTutorApp
    ...
    maybe_run(run_turn_and_render(app=SmartTutorApp(), request=request, fmt=fmt))
...
def start(
    ...
    from smarttutor.runtime.launcher import start as start_web
...
def serve(
    ...
    """Start the Smart Tutor API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from smarttutor.services.setup import get_backend_port
    ...
    try:
        ...
        console.print(
            ...
            "Run: pip install -U smarttutor"
        )
        raise typer.Exit(code=1)

    from smarttutor.services.config import HTTP_KEEP_ALIVE_TIMEOUT, get_ws_max_size
