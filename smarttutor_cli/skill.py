...
``eduhub`` (SmartTutor's native open skill registry). Installs run the full
...
def _resolve_token(explicit: str | None, hub: str) -> str | None:
    ...
    from smarttutor.services.skill import credentials

    return (
        explicit
        or os.environ.get("SMARTTUTOR_HUB_TOKEN")
...
def _summary_table(
    ...
    *,
    ...
    from smarttutor.services.skill.taxonomy import domain_label, track_label
...
def register(app: typer.Typer) -> None:
    ...
    def skill_search(
        ...
        """Search a skill hub."""
        from smarttutor.services.skill.hub import HubError, default_hub, get_hub_provider
        ...
        console.print(table)
        console.print("[dim]Install with: smarttutor skill install <ref>[/]")
    ...
    def skill_install(
        ...
        from smarttutor.services.skill.hub import HubError, install_from_hub
    ...
    def skill_login(
        ...
        from smarttutor.services.skill import credentials
        from smarttutor.services.skill.hub import HubError, default_hub, get_hub_provider
        from smarttutor.services.skill.taxonomy import Option
    ...
    def skill_logout(
        ...
        """清除本地保存的某个 hub 登录令牌。"""
        from smarttutor.services.skill import credentials
        from smarttutor.services.skill.hub import default_hub
    ...
    def skill_publish(
        ...
        token: str | None = typer.Option(
            None, "--token", help="Publish token; else env / `smarttutor skills login`."
        ),
        ...
        from smarttutor.services.skill.hub import (
            HubError,
            default_hub,
            preflight_skill_dir,
            publish_to_hub,
        ...
        from smarttutor.services.skill.service import SkillImportError
        ...
        if not pre.ok:
            ...
            console.print("  [dim]格式规范见 https://eduhub.smarttutor.info/skill-format.md[/]")
            raise typer.Exit(code=1)
        console.print(
            f"[green]✓[/] 格式预检通过 "
        ...
        if not tok:
            console.print(
                "[bold red]未登录。[/] 运行 [bold]smarttutor skills login[/] 完成浏览器授权，"
                "或用 --token / $SMARTTUTOR_HUB_TOKEN 传入令牌。"
            )
            raise typer.Exit(code=1)
        ...
        console.print(
            f"  [dim]install with:[/] smarttutor skill install {outcome.hub}:{outcome.slug}"
        )
    ...
    def skill_list() -> None:
        ...
        from smarttutor.services.skill.service import get_skill_service
