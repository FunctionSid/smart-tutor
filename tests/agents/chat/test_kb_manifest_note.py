...
from smarttutor.knowledge.manifest import KbDocument, KbManifest
...
def _stub_resolver(
    ...
    monkeypatch.setattr(
        "smarttutor.multi_user.knowledge_access.resolve_kb_manifest", _resolve, raising=False
    )
    return asked
...
async def test_unreadable_kb_costs_the_manifest_not_the_turn(
    ...
    monkeypatch.setattr(
        "smarttutor.multi_user.knowledge_access.resolve_kb_manifest", _boom, raising=False
    )
    context = UnifiedContext(
