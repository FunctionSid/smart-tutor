...
from smarttutor.agents.chat.chat_agent import ChatAgent
...
def _fake_llm_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.agents.base_agent.get_llm_config", lambda: cfg)
...
def test_agentic_chat_final_prompt_uses_selected_language(
    ...
    monkeypatch.setattr(
        "smarttutor.agents.chat.agentic_pipeline.get_tool_registry",
        lambda: FakeRegistry(),
    )
    ...
    assert "你是 SmartTutor" in zh_prompt
    assert "You are SmartTutor" in en_prompt
...
def test_legacy_chat_agent_system_prompt_uses_selected_language() -> None:
    ...
    assert "你是 SmartTutor" in zh_messages[0]["content"]
    assert "请严格使用中文" in zh_messages[0]["content"]
    assert "You are SmartTutor" in en_messages[0]["content"]
