...
from smarttutor.runtime.registry.deferred_tools import render_deferred_tools_manifest
...
def _prepare(monkeypatch, docs: dict[str, dict[str, str]]) -> AgenticChatPipeline:
    ...
    monkeypatch.setattr("smarttutor.services.mcp.get_mcp_manager", lambda: FakeManager())
    monkeypatch.setattr("smarttutor.services.mcp.load_loaded_tools", lambda _sid: set())
    # Non-admin user without an MCP grant: fail-closed empty whitelist.
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_mcp_tools", lambda: set())
    monkeypatch.setattr(
        "smarttutor.services.rag.pipelines.pageindex.is_pageindex_kb",
