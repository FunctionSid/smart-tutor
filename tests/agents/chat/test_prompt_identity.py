...
PROMPTS = {
    "general": "You are SmartTutor, an interactive tutor.",
...
def test_chat_turn_keeps_product_identity():
    ...
    assert content == "You are SmartTutor, an interactive tutor."
...
def test_partner_identity_replaces_general():
    ...
    assert "SmartTutor" not in content
...
def test_blank_identity_falls_back_to_product():
    ...
    assert _general_block(context) == "You are SmartTutor, an interactive tutor."


def test_shipped_yaml_carries_partner_templates():
    root = Path(__file__).resolve().parents[3] / "smarttutor/agents/chat/prompts"
    for lang in ("en", "zh"):
