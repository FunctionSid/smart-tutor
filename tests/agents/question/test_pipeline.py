...
from smarttutor.agents.question.pipeline import (
...
def tmp_sqlite_store(tmp_path: Path):
    ...
    with patch(
        "smarttutor.services.session.sqlite_store.get_sqlite_session_store",
        return_value=store,
    ):
        yield store
...
def test_run_with_templates_override_skips_explore_and_plan(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.agents.question.pipeline.build_openai_client",
...
def test_parse_exam_paper_to_templates_happy_path(monkeypatch, tmp_path: Path) -> None:
    ...
    populated."""
    from smarttutor.agents.question import mimic_source
