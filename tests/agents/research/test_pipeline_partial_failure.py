...
from smarttutor.agents.research.pipeline import ResearchedBlock, ResearchPipeline, SubTopicItem
...
def _make_pipeline() -> ResearchPipeline:
    with (
        patch("smarttutor.agents.research.pipeline.get_llm_config", lambda: _FakeLLM()),
        patch("smarttutor.agents.research.pipeline.get_tool_registry", lambda: _FakeRegistry()),
    ):
...
async def _run(pipeline: ResearchPipeline) -> dict:
    ...
    with patch("smarttutor.agents.research.pipeline.emit_capability_result", fake_emit):
