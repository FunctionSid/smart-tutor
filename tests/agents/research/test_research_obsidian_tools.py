...
from smarttutor.agents.research.data_structures import DynamicTopicQueue, ToolTrace
from smarttutor.agents.research.pipeline import (
    LABEL_FINISH,
    ResearchedBlock,
    ResearchPipeline,
    _BlockLoopHost,
...
def _bind_kb(
    ...
    *,
    ...
    monkeypatch.setattr("smarttutor.multi_user.knowledge_access.resolve_kb_metadata", fake)
    return calls
...
def _make_pipeline(
    ...
    *,
    ...
    monkeypatch.setattr("smarttutor.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr("smarttutor.agents.research.pipeline.get_tool_registry", lambda: registry)
    monkeypatch.setattr("smarttutor.agents.research.pipeline.user_has_memory", lambda: False)
    monkeypatch.setattr("smarttutor.agents.research.pipeline.user_has_notebooks", lambda: False)
    monkeypatch.setattr(
        "smarttutor.agents.research.pipeline.exec_capability_available", lambda: False
    )
...
def test_init_does_not_trigger_rag_usage_audit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from smarttutor.multi_user import audit
