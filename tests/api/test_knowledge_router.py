...
import smarttutor.services.config as config_module
...
import smarttutor.services.rag.pipelines.ima.config as ima_config_module
...
if FastAPI is not None and TestClient is not None:
    knowledge_router_module = importlib.import_module("smarttutor.api.routers.knowledge")
...
def _capture_probe(monkeypatch) -> list[tuple[str, str, str]]:
    ...
    import smarttutor.services.rag.pipelines.ima.probe as probe_module
...
def test_create_preserves_known_nondefault_provider(monkeypatch, tmp_path: Path) -> None:
    ...
    pageindex_config = importlib.import_module("smarttutor.services.rag.pipelines.pageindex.config")
...
def test_upload_task_marks_provider_failures_as_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.knowledge.add_documents.RAGService",
        _FailingRagService,
    )
...
def test_reindex_accepts_default_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    embedding_signature = importlib.import_module("smarttutor.services.rag.embedding_signature")
    index_versioning = importlib.import_module("smarttutor.services.rag.index_versioning")
    monkeypatch.setattr(
...
def test_update_config_coerces_legacy_provider_to_llamaindex() -> None:
    ...
    config_module = importlib.import_module("smarttutor.services.config")
    app = _build_app()
...
def _patch_server_probe(monkeypatch, *, ok: bool, error: str | None = None) -> None:
    ...
    from smarttutor.services.rag.pipelines.lightrag_server import probe as probe_module
...
def test_create_pageindex_oss_persists_optional_mode(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(knowledge_router_module, "KnowledgeBaseInitializer", _FakeInitializer)
    monkeypatch.setattr(knowledge_router_module, "_kb_base_dir", tmp_path / "knowledge_bases")
    preflight = importlib.import_module("smarttutor.services.rag.preflight")
...
def test_upload_progress_counts_completed_files_and_reports_reliable_stages(
    ...
    def _record_update(self, stage, message="", current=0, total=0, **kwargs):
        ...
        # this test is about.
        from smarttutor.knowledge.progress_tracker import render_message_template
...
def test_lightrag_config_endpoint_round_trips_the_indexing_knobs(
    ...
    monkeypatch.setattr(
        "smarttutor.services.config.get_runtime_settings_service",
        lambda: service,
    )
