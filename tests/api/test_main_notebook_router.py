...
notebook_router = importlib.import_module("smarttutor.api.routers.notebook").router

from smarttutor.services.notebook.service import NotebookManager
...
def manager(tmp_path, monkeypatch) -> NotebookManager:
    ...
    monkeypatch.setattr(
        "smarttutor.api.routers.notebook.notebook_manager",
        instance,
    )
    return instance
...
def test_stream_add_record_with_summary_strips_thinking_tags(
    ...
    monkeypatch.setattr(
        "smarttutor.api.routers.notebook.NotebookSummarizeAgent",
        FakeSummarizeAgent,
    )
    nb = manager.create_notebook(name="My Notes")

    async def collect_events() -> list[dict]:
        request = importlib.import_module("smarttutor.api.routers.notebook").AddRecordRequest(
            notebook_ids=[nb["id"]],
            record_type="chat",
        ...
        async for raw in importlib.import_module(
            "smarttutor.api.routers.notebook"
