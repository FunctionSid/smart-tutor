...
from smarttutor.api.routers import mcp_settings
from smarttutor.api.routers.auth import require_admin
from smarttutor.services.mcp.config import MCPConfig, MCPServerConfig
...
def admin_client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr(
        "smarttutor.services.mcp.config.mcp_config_path",
        lambda: config_path,
    )
