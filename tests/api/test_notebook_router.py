...
notebook_router = importlib.import_module("smarttutor.api.routers.question_notebook").router
sessions_router = importlib.import_module("smarttutor.api.routers.sessions").router

from smarttutor.services.session.sqlite_store import SQLiteSessionStore
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "smarttutor.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "smarttutor.api.routers.sessions.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "smarttutor.learning.storage.get_path_service",
        lambda: _PathService(),
    )
    return instance
...
def test_quiz_results_update_learning_progress(
    store: SQLiteSessionStore, tmp_path: Path
) -> None:
    from smarttutor.learning.storage import LearningStore

    session = asyncio.run(store.create_session(title="Derivatives Quiz"))
    sid = session["id"]
    answers = [
        {
            **item,
            "concentration": "Differentiation rules",
            "knowledge_context": "Calculus chapter 3",
        }
        for item in _quiz_answers()
    ]

    with TestClient(_build_app(store)) as client:
        resp = client.post(f"/api/v1/sessions/{sid}/quiz-results", json={"answers": answers})
        assert resp.status_code == 200
        body = resp.json()
        assert body["progress_updated"] is True
        assert body["knowledge_point_count"] == 1
        assert body["active_error_count"] == 1
        assert body["score_pct"] == 50

        recovery = [
            {
                **answers[1],
                "question_id": f"q_recovery_{idx}",
                "question": f"Differentiate recovery item {idx}",
                "is_correct": True,
            }
        ...
        recovered = client.post(
            f"/api/v1/sessions/{sid}/quiz-results",
            json={"answers": recovery},
        )
        assert recovered.status_code == 200
        recovered_body = recovered.json()
        assert recovered_body["progress_updated"] is True
        assert recovered_body["active_error_count"] == 0
        assert recovered_body["recovered_error_count"] == 1

    progress = LearningStore(root=tmp_path / "workspace" / "learning").load(
        body["progress_path_id"]
    )
    assert progress is not None
    assert len(progress.quiz_attempts) == 6
    assert progress.modules[0].name == "Quiz Assessment: Derivatives Quiz"
    assert progress.modules[0].knowledge_points[0].name == "Differentiation rules"
