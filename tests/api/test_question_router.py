...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("smarttutor.api.routers.question", None)
...
def _fake_config_module() -> types.ModuleType:
    """Stand in for ``smarttutor.services.config``, deferring the rest to the real one.
    ...
    handler lazily imports ``smarttutor.api.routers.auth``, which pulls
    *unrelated* loaders (auth settings, integrations, …) out of this same
    ...
    ``smarttutor.api.routers.auth`` in ``sys.modules`` — so the lazy import was
    a cache hit that never reached this stand-in. Green in a full run, red on
    its own.
    """
    real = importlib.import_module("smarttutor.services.config")
    module = types.ModuleType("smarttutor.services.config")
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("smarttutor.agents")
    fake_agents_question = types.ModuleType("smarttutor.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "smarttutor.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "smarttutor.agents.question", fake_agents_question)

    fake_logging = _package("smarttutor.logging")
    ...
    monkeypatch.setitem(sys.modules, "smarttutor.logging", fake_logging)

    monkeypatch.setitem(sys.modules, "smarttutor.services.config", _fake_config_module())

    fake_llm_package = _package("smarttutor.services.llm")
    fake_llm_config = types.ModuleType("smarttutor.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "smarttutor.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "smarttutor.services.llm.config", fake_llm_config)

    fake_settings_package = _package("smarttutor.services.settings")
    fake_interface_settings = types.ModuleType("smarttutor.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "smarttutor.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "smarttutor.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("smarttutor.tools")
    fake_tools_question = types.ModuleType("smarttutor.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "smarttutor.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "smarttutor.tools.question", fake_tools_question)

    return importlib.import_module("smarttutor.api.routers.question")
