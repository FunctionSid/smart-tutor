...
The real middleware logs through the ``smarttutor.access`` logger, which carries
...
ACCESS_LOGGER = "smarttutor.access"
...
class TestSelectiveAccessLog:
    ...
    def test_200_not_logged(self):
        """200 responses should not produce smarttutor.access log records."""
        app = _build_app_with_middleware()
        with _CaptureAccess() as cap:
            with TestClient(app) as client:
                client.get("/ok")
