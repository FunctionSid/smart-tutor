...
from smarttutor.api.routers import settings as settings_router
from smarttutor.services.codex_auth.contracts import CodexAuthError
...
from smarttutor.services.llm import client as llm_client_module
from smarttutor.services.llm import config as llm_config_module
...
async def test_network_settings_roundtrip_normalizes_cors_origins(
    ...
    payload = settings_router.NetworkSettingsUpdate(
        backend_port=8101,
        frontend_port=3882,
        public_api_base="https://api.example.com/smarttutor",
    ...
    assert response["settings"]["public_api_base"] == "https://api.example.com/smarttutor"
...
async def test_mineru_payload_includes_local_cli_probe(
    ...
    from smarttutor.services.parsing.engines.mineru import backend as mineru_backend
...
async def test_mineru_models_download_start_requires_downloader(
    ...
    from smarttutor.services.parsing.engines.mineru import models as mineru_models
...
def test_voice_provider_choices_include_global_engines() -> None:
    ...
    stt = {item["value"]: item for item in choices["stt"]}

    assert tts["edge_tts"]["is_local"] is True
    assert tts["edge_tts"]["adapter"] == "global_edge_tts"
    assert tts["piper"]["adapter"] == "global_piper"
    assert tts["pyttsx3"]["adapter"] == "global_pyttsx3"
    assert tts["gtts"]["adapter"] == "global_gtts"
    assert tts["windows_speech"]["adapter"] == "windows_system_speech"
    assert stt["faster_whisper"]["is_local"] is True
    assert stt["faster_whisper"]["adapter"] == "global_faster_whisper"
...
def test_auth_disabled_settings_endpoint_does_not_expose_provider_secrets(
    ...
    from smarttutor.api.routers import auth as auth_router
