...
catalog so the web panel can render it in SmartTutor's own UI (no iframe, no
...
from smarttutor.services.skill import hub as hub_module
from smarttutor.services.skill.hub import ClawHubProvider
...
def _mock_provider() -> ClawHubProvider:
                    ...
                    "skills": [
                        {
                            ...
                            "owner": {
                                "displayName": "SmartTutor",
                                "htmlUrl": "https://smarttutor.info",
                            },
                        }
                    ]
                },
        ...
        if path == "/api/v1/skills/socratic-tutor":
            return httpx.Response(
                ...
                json={
                    ...
                    "owner": {"displayName": "SmartTutor", "htmlUrl": "https://smarttutor.info"},
    ...
    return ClawHubProvider(
        "eduhub",
        base_url="https://eduhub.smarttutor.info/api/v1",
        client=httpx.Client(transport=httpx.MockTransport(handler)),
    )


def _build_app() -> FastAPI:
    router = importlib.import_module("smarttutor.api.routers.skills").router
    app = FastAPI()
...
def test_hub_catalog_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.smarttutor.info"
    row = data["skills"][0]
    assert row["slug"] == "socratic-tutor"
    assert row["downloads"] == 8 and row["stars"] == 2
    assert row["owner"] == "SmartTutor"
...
def test_hub_detail_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.smarttutor.info/skills/socratic-tutor"
