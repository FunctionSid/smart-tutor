...
from smarttutor.api.routers import space_mcp
...
def _offline_dns(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.mcp.network.socket.getaddrinfo", _getaddrinfo)
...
def owner(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    monkeypatch.setattr(
        "smarttutor.services.mcp.config.mcp_config_path", lambda: tmp_path / "admin-mcp.json"
    )
    return acting
...
def test_deleting_a_server_also_drops_its_credentials(client: TestClient, tmp_path: Path) -> None:
    ...
    from smarttutor.services.mcp.secrets import configured_fields
...
def test_installing_an_admin_only_entry_is_refused(client: TestClient) -> None:
    from smarttutor.services.mcp.catalog import load_catalog
...
def test_resaving_a_server_keeps_a_credential_it_did_not_re_enter(
    ...
    from smarttutor.services.mcp.secrets import resolve_references, secret_reference
...
def test_testing_a_draft_writes_nothing(client: TestClient, tmp_path: Path) -> None:
    ...
    """
    import smarttutor.services.mcp.manager as manager_module
