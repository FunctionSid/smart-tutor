...
if FastAPI is not None and TestClient is not None:
    subagents_module = importlib.import_module("smarttutor.api.routers.subagents")
...
def client(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "smarttutor.services.subagent.config._settings_path",
        lambda: tmp_path / "subagent.json",
    )

    async def fake_detect():
        from smarttutor.services.subagent.types import DetectResult
...
def test_backend_options_endpoint_shape(client, monkeypatch):
    ...
    from smarttutor.services.subagent.models import BackendOptions, ModelOption
...
def test_message_connection_streams_and_persists(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("smarttutor.services.subagent.get_backend", lambda kind: _FakeBackend())
