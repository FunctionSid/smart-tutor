...
from smarttutor.book.agents.source_explorer import SourceExplorer, _balanced_slice
from smarttutor.book.models import SourceChunk
...
def fake_metadata(monkeypatch):
    ...
    monkeypatch.setattr("smarttutor.multi_user.knowledge_access.resolve_kb_metadata", _resolve)
    return table
...
async def test_pageindex_is_read_by_source_explorer_agent_not_rag(monkeypatch) -> None:
    monkeypatch.setattr(
        "smarttutor.multi_user.knowledge_access.resolve_kb",
        lambda name, **_kwargs: SimpleNamespace(base_dir="/kb", name=name),
    )
    monkeypatch.setattr(
        "smarttutor.services.rag.provider_binding.resolve_bound_provider",
    ...
    monkeypatch.setattr(
        "smarttutor.services.rag.pipelines.pageindex.reasoning.read_pageindex_with_agent",
        read,
    )
    ...
    monkeypatch.setattr("smarttutor.tools.rag_tool.rag_search", no_rag)
