"""Tests for loop-capability registry + ``smarttutor.loop_capabilities`` discovery."""
...
from smarttutor.capabilities.registry import LOOP_CAPABILITIES, discover_external_loop_capabilities
...
def _no_plugins(*, group: str):
    assert group == "smarttutor.loop_capabilities"
    return []
...
def test_all_loop_capabilities_equals_builtins_without_plugins(monkeypatch) -> None:
    from smarttutor.capabilities import registry
