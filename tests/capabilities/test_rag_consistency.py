...
async def test_deep_research_forwards_enabled_tools_and_kb_unchanged() -> None:
    ...
    with (
        patch(
            "smarttutor.agents.research.capability.ResearchPipeline",
            new=_FakePipeline,
        ),
        patch(
            "smarttutor.services.llm.config.get_llm_config",
            return_value=_fake_llm_config(),
        ),
        patch(
            "smarttutor.agents.research.capability.load_config_with_main",
            return_value={},
        ),
    ):
