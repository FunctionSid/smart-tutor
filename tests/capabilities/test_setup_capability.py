...
from smarttutor.agents._shared.tool_composition import ToolMountFlags, compose_enabled_tools
from smarttutor.capabilities import active_loop_capabilities, any_exclusive_capability_active
from smarttutor.capabilities.setup import SETUP_TOOL_NAMES, SetupCapability
from smarttutor.capabilities.setup.apply import apply_setting
from smarttutor.capabilities.setup.binding import message_signals_setup, setup_gaps
from smarttutor.capabilities.setup.jobs import run_job
from smarttutor.capabilities.setup.tools import (
    ApplySettingTool,
    InspectSetupTool,
...
from smarttutor.services.config import settings_spec as spec_module
from smarttutor.services.config.settings_spec import ProbeResult, setting_specs
...
def isolated_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    from smarttutor.services.config import model_catalog, runtime_settings
    from smarttutor.services.path_service import PathService
    from smarttutor.services.settings import interface_settings
...
async def test_apply_personal_setting_persists(isolated_settings: Path) -> None:
    from smarttutor.services.settings.interface_settings import get_ui_settings
...
async def test_apply_rejects_an_engine_that_is_not_installed(
    ...
    """
    from smarttutor.services.config.runtime_settings import get_runtime_settings_service
    from smarttutor.services.parsing.engines import factory
...
async def test_a_non_admin_cannot_write_a_global_row(
    ...
    from smarttutor.capabilities.setup import access
...
async def test_a_hung_endpoint_cannot_hang_the_turn(
    ...
    from smarttutor.services.config.settings_spec import _PROBE_TIMEOUT_SECONDS, _probe_llm
    ...
    import smarttutor.services.llm as llm_module
...
def test_the_first_run_offer_is_not_spent_by_an_ordinary_request(
    ...
    """
    from smarttutor.capabilities.setup.binding import setup_activation
...
def test_unresolvable_identity_refuses_global_writes(
    ...
    monkeypatch.setattr(access, "_is_partner_turn", lambda: False)
    monkeypatch.setattr("smarttutor.multi_user.context.get_current_user", _explode)
...
async def test_a_silent_job_still_emits_progress(
    ...
    """
    from smarttutor.capabilities.setup import jobs
...
def test_setup_jobs_are_never_listed_twice(isolated_settings: Path) -> None:
    from smarttutor.capabilities.setup.jobs import available_jobs
...
def test_system_block_names_the_installs_gaps(isolated_settings: Path) -> None:
    context = UnifiedContext(user_message="帮我配置一下 SmartTutor")
