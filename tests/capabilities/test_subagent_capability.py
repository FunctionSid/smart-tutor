...
from smarttutor.capabilities.subagent import (
    SUBAGENT_TOOL_NAMES,
    ConsultSubagentTool,
    SubagentCapability,
    connection_for_turn,
)
from smarttutor.capabilities.subagent import binding as subagent_binding
...
from smarttutor.services.subagent.config import BackendConfig
from smarttutor.services.subagent.types import ConsultResult, SubagentEvent
...
async def test_consult_streams_events_and_threads_session(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.subagent.get_backend", lambda kind: backend)
...
async def test_session_id_persists_across_turns(monkeypatch, tmp_path) -> None:
    ...
    # the local agent keeps context across SmartTutor's separate messages.
