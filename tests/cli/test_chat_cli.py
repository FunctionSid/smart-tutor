"""CLI smoke tests for the standalone ``smarttutor-cli`` package."""
...
def _install_fake_runtime(monkeypatch, captured_requests: list[TurnRequest]) -> None:
    ...
    monkeypatch.setattr("smarttutor.app.facade.SmartTutorApp.start_turn", _start_turn)
    monkeypatch.setattr("smarttutor.app.facade.SmartTutorApp.stream_turn", _stream_turn)
...
def test_chat_repl_survives_invalid_utf8_input(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor_cli.chat._read_repl_input", _read_input)
...
def test_session_list_command_uses_shared_store(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.app.facade.SmartTutorApp.list_sessions", _list_sessions)
...
def test_start_command_delegates_to_runtime_launcher(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.runtime.launcher.start", _fake_start)
