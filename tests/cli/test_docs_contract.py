...
def _smarttutor_commands() -> list[str]:
    commands: list[str] = []
    pending = ""
    for path in _command_doc_paths():
        for line in path.read_text(encoding="utf-8").splitlines():
            stripped = line.strip()
            if not pending and not stripped.startswith("smarttutor "):
                continue
...
def test_documented_smarttutor_subcommands_exist() -> None:
    top_level = {
        "book",
        "chat",
        "config",
    ...
    for command in _smarttutor_commands():
...
def test_deep_research_examples_include_required_config() -> None:
    examples = [
        command for command in _smarttutor_commands() if "smarttutor run deep_research" in command
    ]
...
def test_docs_do_not_advertise_removed_cli_forms() -> None:
    ...
    assert "smarttutor provider logout" not in text
    assert "smarttutor memory show summary" not in text
