...
try:  # pragma: no cover - import-time wiring
    from smarttutor.multi_user.paths import ADMIN_WORKSPACE_ROOT as _REAL_ADMIN_ROOT
    from smarttutor.multi_user.paths import SYSTEM_ROOT as _REAL_SYSTEM_ROOT
...
def _guard_legacy_multi_user_migration(monkeypatch):
    ...
    monkeypatch.setattr(
        paths, "LEGACY_MULTI_USER_ROOT", Path("/nonexistent/smarttutor-legacy-multi-user")
    )
...
def _isolate_codebuddy_login(monkeypatch):
    ...
    """
    from smarttutor.services import codebuddy_credentials

    monkeypatch.setenv(
        "SMARTTUTOR_CODEBUDDY_AUTH_FILE", str(Path("/nonexistent/codebuddy-auth.info"))
    )
    monkeypatch.setattr(
    ...
    monkeypatch.delenv("SMARTTUTOR_CODEBUDDY_BACKEND", raising=False)
    yield
