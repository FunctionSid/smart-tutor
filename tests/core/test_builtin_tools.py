...
from smarttutor.tools.builtin import (
    BrainstormTool,
    CodeExecutionTool,
    ExecTool,
...
async def test_exec_tool_reports_generated_public_artifacts(
    ...
    import smarttutor.services.sandbox as sandbox_pkg
    import smarttutor.services.sandbox.artifacts as sandbox_artifacts
...
async def test_brainstorm_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "smarttutor.tools.brainstorm", brainstorm=fake_brainstorm)
...
async def test_rag_tool_forwards_query_and_extra_kwargs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "smarttutor.tools.rag_tool", rag_search=fake_rag_search)
...
async def test_web_search_tool_wraps_sync_function(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "smarttutor.tools.web_search", web_search=fake_web_search)
...
async def test_reason_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "smarttutor.tools.reason", reason=fake_reason)
...
async def test_paper_search_tool_formats_papers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "smarttutor.tools.paper_search_tool",
...
async def test_geogebra_analysis_tool_handles_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "smarttutor.agents.vision_solver.vision_solver_agent",
