...
from smarttutor.core.errors import (
    ConfigurationError,
    SmartTutorError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = SmartTutorError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = SmartTutorError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, SmartTutorError)
        assert issubclass(ValidationError, SmartTutorError)
        assert issubclass(ServiceError, SmartTutorError)
