...
:func:`smarttutor.core.agentic.labeled_step.run_labeled_step`.
...
async def test_wrapped_finish_with_adjacent_body_routes_to_final_not_reasoning() -> None:
    ...
    events, result = await _run(
        [
            _chunk("``FINISH``你好！我是 SmartTutor。"),
        ],
        final_meta=final_meta,
    )

    assert result.label == "FINISH"
    assert result.text == "你好！我是 SmartTutor。"
    assert "你好！我是 SmartTutor。" in "".join(_content_texts(events))
    assert _thinking_texts(events) == []
