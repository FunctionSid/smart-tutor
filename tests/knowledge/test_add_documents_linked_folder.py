...
def test_add_documents_reports_content_hash_duplicates(tmp_path: Path) -> None:
    kb_dir = tmp_path / "kb"
    _ready_llamaindex_kb(kb_dir)

    raw_doc = kb_dir / "raw" / "already.md"
    raw_doc.write_text("same bytes", encoding="utf-8")
    duplicate = tmp_path / "upload.md"
    duplicate.write_text("same bytes", encoding="utf-8")

    adder = DocumentAdder(kb_name="kb", base_dir=str(tmp_path))
    file_hash = adder._get_file_hash(raw_doc)
    (kb_dir / "metadata.json").write_text(
        json.dumps({"file_hashes": {"already.md": file_hash}}),
        encoding="utf-8",
    )

    staged = adder.add_documents([str(duplicate), str(tmp_path / "missing.md")])

    assert staged == []
    assert adder.last_staging_summary.staged_count == 0
    assert adder.last_staging_summary.duplicate_count == 1
    assert adder.last_staging_summary.missing_count == 1
    assert adder.last_staging_summary.skipped_count == 2
