...
def test_progress_tracker_persists_skip_metadata(tmp_path) -> None:
    tracker = ProgressTracker("demo-kb", tmp_path)
    ...
    entry = KnowledgeBaseManager(base_dir=str(tmp_path)).get_kb_entry("demo-kb")
    assert entry is not None
    assert entry["last_upload_skipped_count"] == 2
    assert entry["last_upload_duplicate_count"] == 1
