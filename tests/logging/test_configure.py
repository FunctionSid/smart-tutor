...
from smarttutor.logging import LoggingConfig, bind_log_context


@pytest.fixture(autouse=True)
def _clean_logging_handlers():
    configure_module = importlib.import_module("smarttutor.logging.configure")
...
def test_configure_logging_writes_jsonl_and_respects_level(monkeypatch, tmp_path: Path):
    ...
    logger = logging.getLogger("smarttutor.tests.config")
    with bind_log_context(request_id="req-1", task_id="task-1"):
        logger.info("filtered")
        logger.warning("written")
    _flush_root_handlers()

    lines = (tmp_path / "smarttutor.jsonl").read_text(encoding="utf-8").splitlines()
    assert len(lines) == 1
    entry = json.loads(lines[0])
    assert entry["level"] == "WARNING"
    assert entry["logger"] == "smarttutor.tests.config"
...
def test_configure_logging_uses_rotation_settings(monkeypatch, tmp_path: Path):
    ...
    configure_module.configure_logging(force=True)
    logger = logging.getLogger("smarttutor.tests.rotation")
    for index in range(20):
        logger.info("rotation line %02d %s", index, "x" * 40)
    _flush_root_handlers()

    assert (tmp_path / "smarttutor.jsonl").exists()
    assert (tmp_path / "smarttutor.jsonl.1").exists()
