...
def test_set_avatar_persists_through_normalisation(mu_isolated_root, seed_user):
    from smarttutor.multi_user.identity import list_user_info, load_users, set_avatar
...
def test_save_user_preserves_existing_avatar(mu_isolated_root, seed_user):
    from smarttutor.multi_user.identity import load_users, save_user, set_avatar
...
def test_set_avatar_unknown_user_returns_false(mu_isolated_root, seed_user):
    from smarttutor.multi_user.identity import set_avatar
...
def test_avatar_file_roundtrip_and_extension_replacement(mu_isolated_root):
    from smarttutor.multi_user.identity import (
        delete_avatar_file,
        get_avatar_file,
        save_avatar_file,
    )
...
def test_sniff_image_detects_supported_formats_only():
    from smarttutor.api.routers.auth import _sniff_image
...
def test_update_profile_request_validates_marker():
    from smarttutor.api.routers.auth import UpdateProfileRequest
