...
The store is isolated by pointing ``SMARTTUTOR_HOME`` at a temp dir and resetting
...
from smarttutor.capabilities.reading import (
    MATERIAL_ID_KEY,
    MATERIAL_KWARG,
    READING_TOOL_NAMES,
    VIEWPORT_KEY,
...
from smarttutor.capabilities.reading.tools import (
    MaterialOutlineTool,
    ReaderAnnotateTool,
    ReaderGotoTool,
    ReadMaterialTool,
...
def reading_home(monkeypatch, tmp_path: Path):
    """A real per-user store rooted in a temp SMARTTUTOR_HOME."""
    monkeypatch.setenv("SMARTTUTOR_HOME", str(tmp_path))
...
def material(reading_home: Path):
    from smarttutor.reading import ReadingStore
...
def _mode_context(**metadata) -> UnifiedContext:
    from smarttutor.capabilities.reading.capability import MODE_KEY
...
def test_nothing_open_means_no_viewport_seed_and_no_locate_pass() -> None:
    from smarttutor.capabilities.reading.capability import VIEWPORT_KEY
