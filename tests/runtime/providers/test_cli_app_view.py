...
from smarttutor.core.tool_protocol import BaseTool, ToolDefinition, ToolResult
from smarttutor.runtime.providers import ToolScope
from smarttutor.runtime.providers.view import build_tool_view
from smarttutor.services.cli_apps.models import AppRuntime, InstallKind
from smarttutor.services.cli_apps.paths import abi_stamp
from smarttutor.services.cli_apps.state import InstalledApp, record_install
...
def roots(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    from smarttutor.multi_user import paths
...
def no_mcp(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.mcp.get_mcp_manager", lambda: _Manager())
...
def as_admin(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_cli_apps", lambda: None)
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_mcp_tools", lambda: None)
    monkeypatch.setattr("smarttutor.multi_user.tool_access.exec_override", lambda: None)


@pytest.fixture
def as_learner(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    state: dict[str, Any] = {"cli": set(), "mcp": set(), "exec": None}
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_cli_apps", lambda: state["cli"])
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_mcp_tools", lambda: state["mcp"])
    monkeypatch.setattr("smarttutor.multi_user.tool_access.exec_override", lambda: state["exec"])
    return state
...
def test_the_app_is_loadable_and_dispatchable(as_admin: None, monkeypatch) -> None:
    ...
    async def _fake_run(app, args, **kwargs):
        from smarttutor.services.sandbox.spec import ExecResult

        return ExecResult(stdout="ran")

    monkeypatch.setattr("smarttutor.services.cli_apps.provider.run_app", _fake_run)
