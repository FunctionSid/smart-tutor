...
from smarttutor.runtime.providers.scope import ToolScope
from smarttutor.runtime.providers.view import build_tool_view
from smarttutor.runtime.registry.scoped_registry import ScopedToolRegistry
from smarttutor.runtime.registry.tool_registry import ToolRegistry
...
def _stub_providers(monkeypatch) -> _FakeManager:
    ...
    monkeypatch.setattr("smarttutor.services.mcp.get_mcp_manager", lambda: manager)
    monkeypatch.setattr("smarttutor.services.mcp.load_loaded_tools", lambda session_id: set())
...
def _grant(monkeypatch, value: set[str] | None) -> None:
    monkeypatch.setattr("smarttutor.multi_user.tool_access.allowed_mcp_tools", lambda: value)
...
async def test_a_failing_provider_degrades_instead_of_killing_the_turn(
    ...
    monkeypatch.setattr("smarttutor.services.mcp.get_mcp_manager", _boom)
    view = await build_tool_view(base_registry=registry, scope=ToolScope(session_id="s"))
    assert view.loader is None
    assert view.pool == ()
...
async def test_attach_adds_loaded_schemas_and_binds_the_live_list(registry, monkeypatch) -> None:
    _grant(monkeypatch, None)
    monkeypatch.setattr(
        "smarttutor.services.mcp.load_loaded_tools", lambda session_id: {"mcp_gh_search"}
    )
    view = await build_tool_view(base_registry=registry, scope=ToolScope(session_id="s"))
...
async def test_a_slow_personal_server_costs_only_its_own_tools(
    ...
    A third-party host that hangs must not present to the user as SmartTutor
    hanging, so the scope connect is bounded and the turn proceeds without it.
    """
    from smarttutor.runtime.providers import view as view_module
