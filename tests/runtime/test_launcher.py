...
from smarttutor.runtime import launcher
from smarttutor.runtime.home import validate_runtime_home
...
def test_runtime_home_rejects_project_data_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("smarttutor.runtime.home.PACKAGE_ROOT", package_root)

    with pytest.raises(ValueError, match="Invalid SmartTutor runtime home"):
...
def test_start_does_not_create_nested_data_tree(monkeypatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit, match="Invalid SmartTutor runtime home"):
...
def test_detect_existing_source_frontend_falls_back_to_windows_process_scan(
    ...
    monkeypatch.setattr(launcher.subprocess, "run", lambda *_args, **_kwargs: _Completed())
    monkeypatch.setattr(launcher, "_windows_pid_listen_ports", lambda pid: [3000])
    ...
    assert existing is not None
    assert existing.url == "http://localhost:3000"
    assert existing.port == 3000
    assert existing.pid == 456
...
def test_source_production_build_is_reused_until_an_input_changes(
    ...
    def _run(command, cwd, env, **_kwargs):
        calls.append((list(command), Path(cwd), env["SMARTTUTOR_NEXT_DIST_DIR"]))
...
def test_start_uses_ipv4_loopback_for_frontend_proxy(
    ...
    from smarttutor.services import config as config_module
    from smarttutor.services import setup as setup_module
    ...
    assert captured_env["SMARTTUTOR_API_BASE_URL"] == (f"http://127.0.0.1:{resolved_backend_port}")


def test_start_opens_browser_only_after_frontend_ready(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from smarttutor.services import config as config_module
    ...
    monkeypatch.setenv("SMARTTUTOR_OPEN_BROWSER", "1")
    monkeypatch.setattr(launcher, "_relax_console_encoding", lambda: None)
    monkeypatch.setattr(launcher, "_reset_runtime_singletons", lambda: None)
    monkeypatch.setattr(config_module, "ensure_runtime_settings_files", lambda: None)
    monkeypatch.setattr(config_module, "load_launch_settings", lambda _home: settings)
    ...
    def _spawn(_command, *, cwd, env, name):
        events.append(f"spawn:{name}")
        return launcher.ManagedProcess(name, _Proc(), None)

    def _wait_for_http(**kwargs):
        events.append(f"ready:{kwargs['name']}")

    def _open(url):
        events.append(f"open:{url}")
        raise SystemExit(0)

    monkeypatch.setattr(launcher, "_spawn", _spawn)
    monkeypatch.setattr(launcher, "_wait_for_http", _wait_for_http)
    monkeypatch.setattr(launcher.webbrowser, "open", _open)
