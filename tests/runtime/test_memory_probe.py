...
def test_classify_maps_roles_and_falls_back_to_the_executable_name() -> None:
    ...
    assert memory_probe._classify(other, "python", "-m uvicorn smarttutor.api.main:app") == "backend"
...
def test_classify_names_the_tree_root_by_pid_not_by_argv() -> None:
    """The supervisor runs as a bare `smarttutor` script — argv has nothing to match."""
    root = os.getpid() + 1

    assert memory_probe._classify(root, "python3.11", "/usr/bin/smarttutor", root_pid=root) == (
        "supervisor"
    )
    # Without the anchor there is no root to name, so it falls back to the name.
    assert memory_probe._classify(root, "python3.11", "/usr/bin/smarttutor") == "python3.11"
