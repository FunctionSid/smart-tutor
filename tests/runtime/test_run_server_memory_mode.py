...
from smarttutor.api import run_server
...
def uvicorn_kwargs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, Any]:
    ...
    from smarttutor import logging as smarttutor_logging
    from smarttutor.runtime import mode
    from smarttutor.services import setup

    monkeypatch.setattr(smarttutor_logging, "configure_logging", lambda: None)
...
def test_run_server_disables_reload_by_default(
    ...
    monkeypatch.delenv("SMARTTUTOR_DEV_RELOAD", raising=False)
...
def test_run_server_reload_remains_available_for_development(
    ...
    monkeypatch.setenv("SMARTTUTOR_DEV_RELOAD", "true")
    (tmp_path / "data").mkdir()
