...
correct only if *every* launch point passes it, and SmartTutor has five (two
Dockerfile stages, the ``smarttutor start`` launcher, the CLI, run_server). A
...
_LAUNCH_POINTS = [
    ("smarttutor/runtime/launcher.py", '"uvicorn",', _CLI_FLAGS),
    ("smarttutor/api/run_server.py", "uvicorn.run(", _PYTHON_FLAGS),
    ("smarttutor_cli/main.py", "uvicorn.run(", _PYTHON_FLAGS),
]
...
def test_dockerfile_launch_points_wire_serving_flags() -> None:
    ...
    lines = [
        ...
        if "-m uvicorn smarttutor.api.main:app" in line
    ]
...
def test_keep_alive_outlasts_the_proxy_socket_reaper() -> None:
    ...
    """
    from smarttutor.services.config import HTTP_KEEP_ALIVE_TIMEOUT
