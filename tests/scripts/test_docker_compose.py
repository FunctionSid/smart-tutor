...
def test_render_docker_env_reads_json_only(tmp_path: Path) -> None:
    ...
    assert values == {
        "SMARTTUTOR_DOCKER_BACKEND_PORT": "9001",
        "SMARTTUTOR_DOCKER_FRONTEND_PORT": "4000",
        "SMARTTUTOR_DOCKER_POCKETBASE_PORT": "19090",
    }
    saved = output_path.read_text(encoding="utf-8")
    assert "\nBACKEND_PORT=" not in saved
    assert "SMARTTUTOR_DOCKER_BACKEND_PORT=9001" in saved
...
def test_render_docker_env_uses_defaults_for_missing_or_invalid_json(tmp_path: Path) -> None:
    ...
    assert values["SMARTTUTOR_DOCKER_BACKEND_PORT"] == "8001"
    assert values["SMARTTUTOR_DOCKER_FRONTEND_PORT"] == "3782"
    assert values["SMARTTUTOR_DOCKER_POCKETBASE_PORT"] == "8090"
...
def test_codex_oauth_overlay_forwards_loopback_callbacks_to_frontend() -> None:
    ...
    assert set(ports) == {
        "127.0.0.1:1455:${SMARTTUTOR_DOCKER_FRONTEND_PORT:-3782}",
        "127.0.0.1:1457:${SMARTTUTOR_DOCKER_FRONTEND_PORT:-3782}",
    }
...
def test_container_docs_use_temporary_codex_oauth_bridge() -> None:
    ...
    for base_file in ("docker-compose.yml", "docker-compose.ghcr.yml"):
        assert (
            f"-f {base_file} -f compose.codex-oauth.yaml up -d --force-recreate smarttutor"
...
def test_dockerfile_is_json_driven_without_bundle_sed() -> None:
    ...
    (including SMARTTUTOR_API_BASE_URL / SMARTTUTOR_AUTH_ENABLED) and web/proxy.ts
    ...
    assert "SMARTTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1" in content
...
def test_supervisord_runs_as_root_with_unprivileged_children() -> None:
    ...
    supervisord to the unprivileged ``smarttutor`` user via ``gosu`` made child
    ...
    ``user=smarttutor`` directive instead, which keeps them unprivileged in both
    ...
    assert "gosu smarttutor /usr/bin/supervisord" not in content
    # Every supervisord program drops to the unprivileged smarttutor user, so the
    ...
    for block in program_blocks:
        ...
        assert "user=smarttutor" in section, (
            f"supervisord program '{name}' must run as smarttutor (user=smarttutor)"
        )
...
def test_frontend_api_is_url_agnostic_passthrough() -> None:
    ...
    assert "SMARTTUTOR_API_BASE_URL" in proxy_ts
