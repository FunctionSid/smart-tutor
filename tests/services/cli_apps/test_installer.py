...
from smarttutor.services.cli_apps import get_entry
from smarttutor.services.cli_apps.installer import install_app, uninstall_app
from smarttutor.services.cli_apps.models import (
    AppRuntime,
    AppTrust,
    CliAppEntry,
    InstallKind,
...
from smarttutor.services.cli_apps.paths import (
    abi_stamp,
    app_dir,
    bin_dir,
    install_log_path,
...
from smarttutor.services.cli_apps.state import load_installed
...
class _Recorder:
    ...
    def _create_entry_point(argv: list[str]) -> None:
        ...
        for token in argv:
            path = Path(token)
            if path.name in {"python", "python.exe"} and path.parent.name in {"bin", "Scripts"}:
                path.parent.mkdir(parents=True, exist_ok=True)
                (path.parent / "cli-anything-demo").write_text(
                    "#!/bin/sh\n", encoding="utf-8"
                )
                return
...
def run(monkeypatch: pytest.MonkeyPatch) -> _Recorder:
    recorder = _Recorder()
    monkeypatch.setattr("smarttutor.services.cli_apps.installer._run", recorder)
    return recorder
...
def test_a_python_app_gets_its_own_venv_and_the_pinned_requirement(run: _Recorder) -> None:
    ...
    assert Path(run.calls[1][0]).parent == bin_dir("demo", AppRuntime.PYTHON)
...
def test_an_install_that_produces_no_entry_point_is_a_failure(
    ...
    monkeypatch.setattr(
        "smarttutor.services.cli_apps.installer._run", _Recorder(make_executable=False)
    )
...
def test_a_failed_update_puts_the_working_install_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.cli_apps.installer._run", _Recorder())
    assert asyncio.run(install_app(_entry())).ok
    executable = bin_dir("demo", AppRuntime.PYTHON) / "cli-anything-demo"
    executable.write_text("#!/bin/sh\necho original\n", encoding="utf-8")

    monkeypatch.setattr("smarttutor.services.cli_apps.installer._run", _Recorder(fail_at=1))
    outcome = asyncio.run(install_app(_entry()))
