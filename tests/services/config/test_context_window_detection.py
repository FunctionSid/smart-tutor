...
from smarttutor.services.config import context_window_detection as detection_module
...
def test_detect_context_window_prefers_provider_metadata(monkeypatch) -> None:
    monkeypatch.setattr(
        "smarttutor.services.config.context_window_detection._detect_from_models_endpoint",
        _metadata_128k,
    )
