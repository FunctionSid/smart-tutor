...
from smarttutor.services.config import loader as loader_module
from smarttutor.services.config.loader import get_agent_params
...
class TestLlmProbeUsesAgentsYaml:
    ...
    async def test_probe_passes_configured_max_tokens(self, monkeypatch):
        from smarttutor.services import llm as llm_module
        from smarttutor.services.config import test_runner as test_runner_module
        from smarttutor.services.config.test_runner import ConfigTestRunner, TestRun
...
def _stub_resolved_llm(**overrides: Any):
    ...
    from smarttutor.services.config.provider_runtime import ResolvedLLMConfig
...
def _real_get_token_limit_kwargs(model: str, max_tokens: int) -> dict[str, int]:
    ...
    from smarttutor.services.llm.config import uses_max_completion_tokens
...
async def _stub_context_window_detection(*_args, **_kwargs):
    from smarttutor.services.config.context_window_detection import (
