...
def test_render_environment_uses_json_backed_runtime_names(monkeypatch, tmp_path: Path) -> None:
    ...
    # middleware). SMARTTUTOR_AUTH_ENABLED gates the login redirect;
    # SMARTTUTOR_API_BASE_URL is where the frontend server reaches the backend
    ...
    assert env["SMARTTUTOR_AUTH_ENABLED"] == "true"
    assert env["SMARTTUTOR_API_BASE_URL"] == "http://127.0.0.1:8010"
...
def test_api_base_url_falls_back_to_ipv4_loopback(monkeypatch, tmp_path: Path) -> None:
    ...
    assert env["SMARTTUTOR_API_BASE_URL"] == "http://127.0.0.1:8042"
...
def test_api_base_url_prefers_a_configured_base_over_the_loopback(
    ...
    assert env["SMARTTUTOR_API_BASE_URL"] == "http://backend.internal:9000"
...
def test_startup_ensure_creates_missing_runtime_jsons_with_defaults(
    ...
    from smarttutor.services.config import runtime_settings as runtime_settings_module
...
def test_document_parsing_v1_to_v2_migration(tmp_path: Path) -> None:
    ...
    from smarttutor.services.config.runtime_settings import _atomic_write_json
...
def test_compute_ws_max_size_floor_and_inflation() -> None:
    from smarttutor.services.config.runtime_settings import compute_ws_max_size
