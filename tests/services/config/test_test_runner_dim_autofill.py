...
async def test_persist_when_catalog_dim_empty() -> None:
    ...
    with (
        patch(
            "smarttutor.services.config.test_runner.resolve_embedding_runtime_config",
            return_value=_resolved_stub(dim=0),
        ),
        patch("smarttutor.services.embedding.client.EmbeddingClient", return_value=fake_client),
...
async def test_smoke_probe_forces_dim_zero() -> None:
    ...
    with (
        ...
        patch(
            "smarttutor.services.embedding.client.EmbeddingClient",
