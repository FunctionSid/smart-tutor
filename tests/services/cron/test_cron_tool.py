...
from smarttutor.services.cron.service import CronService
from smarttutor.tools.cron_tool import run_cron_action


@pytest.fixture
def cron_service(tmp_path, monkeypatch):
    import smarttutor.services.cron.service as service_mod
    import smarttutor.tools.cron_tool as tool_mod
...
class TestRegistryIntegration:
    def test_cron_tool_is_builtin_and_automounted(self):
        from smarttutor.agents._shared.tool_composition import AUTO_MOUNTED_TOOLS
        from smarttutor.tools.builtin import BUILTIN_TOOL_NAMES

        assert "cron" in BUILTIN_TOOL_NAMES
        assert "cron" in AUTO_MOUNTED_TOOLS

    def test_schema_has_action_enum(self):
        from smarttutor.tools.builtin import CronTool
...
class TestExecutorRouting:
    ...
    async def test_partner_job_runs_and_publishes_outbound(self, monkeypatch):
        from smarttutor.services.cron import executor
        from smarttutor.services.cron.service import CronJob, CronOwner, CronSchedule
        ...
        import smarttutor.services.partners as partners_mod
