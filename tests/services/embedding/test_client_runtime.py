...
from smarttutor.services.embedding.client import (
    EmbeddingClient,
...
async def test_embedding_client_batches_requests(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.embedding.client._resolve_adapter_class", lambda _b: _FakeAdapter
    )
...
async def test_embedding_client_forwards_input_type_to_every_batch(monkeypatch) -> None:
    ...
    _FakeAdapter.instances = []
    monkeypatch.setattr(
        "smarttutor.services.embedding.client._resolve_adapter_class", lambda _b: _RoleAwareAdapter
    )
    client = EmbeddingClient(_build_config("openai"))
...
async def test_embedding_client_rejects_null_vector_values(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.embedding.client._resolve_adapter_class",
...
def test_get_embedding_client_refreshes_when_config_changes(monkeypatch) -> None:
    from smarttutor.services.embedding import client as client_module
