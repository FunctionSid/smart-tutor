...
from smarttutor.services.embedding.adapters.base import (
    EmbeddingProviderError,
    EmbeddingRequest,
)
from smarttutor.services.embedding.adapters.openai_sdk import (
...
def stub_client(monkeypatch: pytest.MonkeyPatch) -> type[_ClientStub]:
    ...
    monkeypatch.setattr(
        "smarttutor.services.embedding.adapters.openai_sdk.AsyncOpenAI",
        _ClientStub,
    )
    return _ClientStub
...
async def test_embed_forwards_extra_headers(stub_client: type[_ClientStub]) -> None:
    adapter = _make_adapter(extra_headers={"X-App": "smarttutor"})
    await adapter.embed(EmbeddingRequest(texts=["x"], model="text-embedding-3-large"))
    assert stub_client.constructor_kwargs["default_headers"] == {"X-App": "smarttutor"}
