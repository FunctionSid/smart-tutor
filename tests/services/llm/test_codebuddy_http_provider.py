...
from smarttutor.services.codebuddy_credentials import (
    INTERNAL_ENDPOINT,
    CodeBuddyAuthUnavailable,
)
from smarttutor.services.llm.provider_core import codebuddy_http_provider as http_module
from smarttutor.services.llm.provider_core.base import LLMResponse
...
def _sign_in(tmp_path: Path, monkeypatch, *, expires_in: float = 86400.0) -> Path:
    ...
    monkeypatch.setenv("SMARTTUTOR_CODEBUDDY_AUTH_FILE", str(path))
    return path
...
def _capture_stream(monkeypatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr(
        "smarttutor.services.llm.provider_core.openai_compat_provider."
...
async def test_blocking_chat_streams_because_cloud_rejects_non_stream(
    ...
    monkeypatch.setattr(
        "smarttutor.services.llm.provider_core.openai_compat_provider.OpenAICompatProvider.chat",
        fail_chat,
    )
...
@pytest.mark.asyncio
async def test_provider_strips_smarttutor_session_id(tmp_path, monkeypatch) -> None:
    _sign_in(tmp_path, monkeypatch)
    captured = _capture_stream(monkeypatch)

    await CodeBuddyHTTPProvider().chat_stream(
        messages=[{"role": "user", "content": "hi"}],
        smarttutor_session_id="session-1",
    )

    assert "smarttutor_session_id" not in captured
