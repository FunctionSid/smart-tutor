...
from smarttutor.services.llm.provider_core import CodeBuddyProvider
from smarttutor.services.llm.provider_core.codebuddy_provider import fetch_codebuddy_models
from smarttutor.services.llm.provider_factory import get_runtime_provider
...
async def test_codebuddy_provider_maps_sdk_mcp_tool_calls(monkeypatch) -> None:
    ...
    assert option_kwargs["tools"] == ["mcp__smarttutor__web_search"]
    assert "smarttutor" in option_kwargs["mcp_servers"]
...
async def test_codebuddy_session_survives_cross_task_turns(monkeypatch) -> None:
    ...
    async def turn(content: str) -> str:
        response = await provider.chat(
            ...
            smarttutor_session_id="cross-task",
        )
...
async def test_codebuddy_provider_reuses_session_and_sends_message_delta(monkeypatch) -> None:
    ...
    first = await provider.chat(initial, smarttutor_session_id="chat-1")
...
async def test_codebuddy_session_drains_interrupt_before_tool_result_round(monkeypatch) -> None:
    ...
    tool_response = await provider.chat(
        initial,
        tools=tools,
        smarttutor_session_id="chat-tools",
    )
...
def test_codebuddy_registry_aliases_and_factory(monkeypatch) -> None:
    ...
    monkeypatch.setenv("SMARTTUTOR_CODEBUDDY_BACKEND", "sdk")
    monkeypatch.setattr(
        "smarttutor.services.llm.provider_core.codebuddy_http_provider.sdk_installed",
        lambda: True,
    )
...
def test_codebuddy_ignores_smarttutor_no_key_placeholder() -> None:
