...
from smarttutor.services.llm.factory import complete, stream
...
async def test_complete_merges_config_and_caller_extra_headers(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.llm.factory.get_llm_config", lambda: cfg)
    ...
    monkeypatch.setattr(
        "smarttutor.services.llm.factory.get_runtime_provider", _fake_get_runtime_provider
    )
...
async def test_explicit_call_inherits_matching_profile_headers_and_reasoning(
    ...
    cfg = _make_cfg(
        extra_headers={"User-Agent": "SmartTutor-Test"},
    ...
    assert result == "ok"
    assert captured_config["config"].extra_headers == {"User-Agent": "SmartTutor-Test"}
