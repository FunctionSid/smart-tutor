...
from smarttutor.services.llm.providers.routing import RoutingProvider
...
def test_routing_provider_local_complete(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.llm.local_provider.complete",
...
def test_routing_provider_cloud_complete(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.llm.cloud_provider.complete",
...
def test_routing_provider_stream(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.llm.local_provider.stream", _fake_stream)
