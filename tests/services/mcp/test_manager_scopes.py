...
from smarttutor.services.mcp.config import MCPServerConfig
...
def _write_user_servers(owner: str, **urls: str) -> None:
    from smarttutor.services.mcp.user_config import save_user_server
...
def _offline_dns(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.mcp.network.socket.getaddrinfo",
...
async def test_a_stdio_entry_in_a_users_file_is_never_connected(
    ...
    from smarttutor.services.mcp.user_config import user_mcp_path
...
async def test_removing_a_server_disconnects_it(manager: MCPConnectionManager) -> None:
    from smarttutor.services.mcp.user_config import delete_user_server
...
async def test_an_idle_scope_is_dropped_when_another_arrives(
    ...
    """
    from smarttutor.services.mcp import manager as manager_module
