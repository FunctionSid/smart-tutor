...
from smarttutor.services.parsing.engines.docling.config import DoclingConfig
from smarttutor.services.parsing.engines.tika.config import TikaConfig
from smarttutor.services.parsing.types import ParserError
...
def test_text_only_parser_extracts_docx_text(tmp_path) -> None:
    ...
    with zipfile.ZipFile(docx, "w") as zf:
        zf.writestr(
            ...
            <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
              <w:body>
                <w:p><w:r><w:t>Hello SmartTutor</w:t></w:r></w:p>
              </w:body>
            </w:document>
            """.strip(),
        )
    ...
    assert (workdir / "lesson.md").read_text(encoding="utf-8") == "Hello SmartTutor"


def test_mineru_signature_distinguishes_local_and_cloud() -> None:
    parser = factory.get_parser("mineru")
    from smarttutor.services.parsing.engines.mineru.config import MinerUConfig
...
def test_mineru_cloud_readiness_needs_token() -> None:
    from smarttutor.services.parsing.engines.mineru.config import MinerUConfig
    from smarttutor.services.parsing.engines.mineru.readiness import mineru_readiness
...
def test_mineru_local_model_download_gate(monkeypatch: pytest.MonkeyPatch) -> None:
    from smarttutor.services.parsing.engines.mineru import backend
    from smarttutor.services.parsing.engines.mineru import readiness as rd
    from smarttutor.services.parsing.engines.mineru.config import MinerUConfig
...
def test_pymupdf4llm_signature_tracks_image_knobs() -> None:
    ...
    from smarttutor.services.parsing.engines.pymupdf4llm.config import PyMuPDF4LLMConfig
...
def test_pymupdf4llm_parses_pdf_and_extracts_images(tmp_path) -> None:
    ...
    doc = pymupdf.open()
    page = doc.new_page()
    page.insert_text((72, 72), "Hello SmartTutor via PyMuPDF4LLM")
...
def test_liteparse_signature_tracks_knobs() -> None:
    ...
    from smarttutor.services.parsing.engines.liteparse.config import LiteParseConfig
...
def test_liteparse_leaves_foreign_image_links_alone(tmp_path, monkeypatch) -> None:
    ...
    from smarttutor.services.parsing.engines.liteparse.engine import LiteParseParser
...
def test_resolve_model_downloader_unknown_engine() -> None:
    from smarttutor.services.parsing.engines._install import resolve_model_downloader
...
def test_docling_models_dir_honors_cache_env(monkeypatch, tmp_path) -> None:
    from smarttutor.services.parsing.engines.docling import engine as docling_engine
