...
from smarttutor.partners.channels import msteams as msteams_mod
from smarttutor.partners.channels.msteams import (
...
class TestSanitizeInboundText:
    ...
    def test_strips_bot_mention_markup(self, state_dir):
        ch = _make_channel()
        out = ch._sanitize_inbound_text(_activity(text="<at>SmartTutor</at> explain entropy"))
...
class TestHandleActivity:
    ...
    async def test_mention_only_text_uses_fallback_response(self, state_dir):
        ch = _make_channel()
        await ch._handle_activity(_activity(text="<at>SmartTutor</at>"))
        msg = ch.bus.publish_inbound.call_args[0][0]
