...
from smarttutor.partners.bus.events import InboundMessage
from smarttutor.partners.bus.queue import MessageBus
from smarttutor.services.partners.manager import PartnerConfig
...
class _FakeOrchestrator:
    ...
    async def handle(self, context):
        from smarttutor.services.memory.paths import memory_root
...
def fake_orchestrator(monkeypatch):
    import smarttutor.runtime.orchestrator as orch_mod
    from smarttutor.services.model_selection import runtime as selection_runtime
...
def _runner(partners_root, config: PartnerConfig | None = None) -> PartnerRunner:
    from smarttutor.partners.config.paths import get_partner_sessions_dir
...
class TestTurnExecution:
    ...
    async def test_llm_config_error_folds_into_graceful_reply(
        ...
        from smarttutor.services.llm.exceptions import LLMConfigError
        from smarttutor.services.model_selection import runtime as selection_runtime
...
class TestContextAssembly:
    ...
    async def test_context_carries_soul_tools_and_metadata(self, partners_root, fake_orchestrator):
        from smarttutor.services.partners.workspace import write_soul
    ...
    async def test_default_tools_resolve_to_full_toggleable_set(
        ...
        from smarttutor.agents._shared.tool_composition import default_optional_tools
...
class TestBuiltinToolsAndMemory:
    ...
    async def test_turn_runs_against_partner_memory(self, partners_root, fake_orchestrator):
        ...
        from smarttutor.partners.config.paths import get_partner_workspace
    ...
    async def test_authenticated_users_get_isolated_session_history(
        ...
        from smarttutor.multi_user.models import CurrentUser
        from smarttutor.multi_user.paths import scope_for_user
        from smarttutor.partners.config.paths import get_partner_user_sessions_dir
...
class TestLiveTurn:
    def test_buffer_replays_for_late_subscriber(self):
        from smarttutor.services.partners.manager import LiveTurn
    ...
    async def test_web_turn_runs_on_instance_and_survives_resubscribe(
        ...
        from smarttutor.services.partners.manager import PartnerManager
    ...
    async def test_manager_captures_authenticated_actor_for_private_history(
        ...
        from smarttutor.multi_user.models import CurrentUser
        from smarttutor.multi_user.paths import scope_for_user, user_context
        from smarttutor.partners.config.paths import get_partner_user_sessions_dir
        from smarttutor.services.partners.manager import PartnerManager
...
class TestPartnerCommands:
    ...
    async def test_sessions_resume_delete_commands(self, partners_root, fake_orchestrator):
        from smarttutor.services.partners.commands import PartnerCommandHandler
    ...
    async def test_telegram_bot_command_suffix_is_supported(self, partners_root, fake_orchestrator):
        ...
        reply = await runner.process_message(_msg("/new@SmartTutorBot"))
