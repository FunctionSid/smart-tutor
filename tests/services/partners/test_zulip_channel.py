...
from smarttutor.partners.channels.zulip import ZulipChannel, ZulipConfig
...
class TestIsMentioned:
    ...
    def test_content_fallback_when_flags_empty(self):
        ...
        ch = _make_channel()
        ch._bot_user_id = 100
        ch._bot_full_name = "SmartTutor Bot"
        assert ch._is_mentioned({"flags": [], "content": "hi @**SmartTutor Bot** help"}) is True

    def test_content_fallback_requires_full_name(self):
        ch = _make_channel()
        ch._bot_user_id = 100
        ch._bot_full_name = ""
        assert ch._is_mentioned({"flags": [], "content": "hi @**SmartTutor Bot**"}) is False
    ...
    def test_content_fallback_disambiguated_mention(self):
        ...
        ch._bot_user_id = 100
        ch._bot_full_name = "SmartTutor Bot"
        assert ch._is_mentioned({"flags": [], "content": "hi @**SmartTutor Bot|100** help"}) is True
...
class TestDownloadAttachments:
    def test_extracts_from_markdown_content(self, tmp_path: Path):
        ...
        with (
            ...
            patch("smarttutor.partners.channels.zulip.requests.get") as mock_get,
        ):
            mock_resp = MagicMock()
...
class TestSend:
    ...
    async def test_send_text_uses_call_endpoint(self):
        ...
        from smarttutor.partners.bus.events import OutboundMessage
...
class TestSubscribeStreams:
    ...
    def test_subscribe_partial_failure_logs_warning(self):
        ...
        with patch("smarttutor.partners.channels.zulip.logger.warning") as mock_warning:
            ch._subscribe_to_streams()
    ...
    def test_subscribe_non_success_all_already_subscribed_is_debug_only(self):
        ...
        with (
            patch("smarttutor.partners.channels.zulip.logger.warning") as mock_warning,
            patch("smarttutor.partners.channels.zulip.logger.debug") as mock_debug,
        ):
            ch._subscribe_to_streams()
...
class TestStart:
    ...
    async def test_start_profile_failure(self, monkeypatch):
        ...
        with patch("smarttutor.partners.channels.zulip.ZulipChannel._call_with_retry") as mock_retry:
