"""Regression tests for SmartTutor's GraphRAG completion compatibility seam."""
...
from smarttutor.services.rag.pipelines.graphrag import engine
...
def test_settings_route_deepseek_completion_without_changing_embedding() -> None:
    ...
    assert completion["type"] == "smarttutor_litellm"
...
def test_real_graphrag_completion_falls_back_when_schema_type_is_unavailable(
    ...
    from smarttutor.services.rag.pipelines.graphrag import completion_adapter
...
def test_old_settings_are_adapted_in_memory_without_rewrite(tmp_path: Path) -> None:
    ...
    assert loaded_completion.type == "smarttutor_litellm"
    assert loaded_completion.model_provider == "deepseek"
    assert settings_path.read_bytes() == before


def test_provider_resolution_uses_backend_contracts() -> None:
    from smarttutor.services.rag.pipelines.graphrag.provider import (
