...
from smarttutor.services.rag.pipelines.graphrag import config as gr_config
from smarttutor.services.rag.pipelines.graphrag import engine, ingestion, storage
from smarttutor.services.rag.pipelines.graphrag.errors import (
...
from smarttutor.services.rag.pipelines.graphrag.pipeline import GraphRagPipeline, _context_to_sources
...
def test_build_settings_reuses_smarttutor_reasoning_request_options(
    binding: str,
    model: str,
...
def test_build_settings_does_not_guess_compatibility_from_model_name(monkeypatch) -> None:
    monkeypatch.setattr(
        "smarttutor.services.llm.capabilities.supports_response_format",
...
def test_compatibility_probe_resolves_candidate_without_mutating_active_model(
    ...
    from smarttutor.services.rag.pipelines.graphrag import compatibility
...
def test_embedding_preflight_uses_graphrag_client_and_checks_dimension(monkeypatch) -> None:
    ...
    assert captured == {
        "input": ["SmartTutor GraphRAG embedding compatibility test"],
        "timeout": 25,
    }
...
def test_ingestion_uses_active_parse_service_for_parser_files(tmp_path, monkeypatch) -> None:
    from smarttutor.services import parsing
...
def _stub_build(monkeypatch) -> list[dict]:
    ...
    monkeypatch.setattr(
        "smarttutor.services.config.resolve_llm_runtime_config",
        lambda: _Cfg("gpt-4o-mini", "https://llm.test/v1", "sk-llm"),
    )
    monkeypatch.setattr(
        "smarttutor.services.embedding.get_embedding_config",
...
def test_router_provider_preflight_does_not_require_a_model_probe() -> None:
    from smarttutor.api.routers import knowledge
...
def test_router_graphrag_readiness_does_not_make_a_paid_model_call(monkeypatch) -> None:
    from smarttutor.api.routers import knowledge
    from smarttutor.services.rag import preflight
    from smarttutor.services.rag.pipelines.graphrag import compatibility
