...
* :mod:`~smarttutor.services.rag.pipelines.ima.inventory` — folder traversal,
  request budget, and the cache that keeps a per-turn manifest from re-fetching;
* :mod:`smarttutor.knowledge.manifest` — an ``ima`` KB now enumerates for real,
...
from smarttutor.knowledge.manifest import (
    UNAVAILABLE_REMOTE,
    build_manifest,
    render_manifest_note,
    render_manifest_report,
)
from smarttutor.services.rag.pipelines.ima import inventory as inventory_module
from smarttutor.services.rag.pipelines.ima.inventory import (
    MAX_REQUESTS,
    ImaInventory,
    clear_cache,
    read_inventory,
)
from smarttutor.services.rag.pipelines.ima.models import ImaDocument, ImaFolder, ImaKnowledgePage
...
class TestTraversal:
    ...
    def test_the_production_path_drives_the_real_client(
        ...
        from smarttutor.services.rag.pipelines.ima import client as client_module
...
def _patch_reader(monkeypatch: pytest.MonkeyPatch, result) -> None:
    ...
    from smarttutor.knowledge import manifest as manifest_module
