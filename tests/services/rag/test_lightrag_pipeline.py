...
from smarttutor.services.llm.exceptions import (
    LLMAPIError,
...
from smarttutor.services.rag.index_versioning import resolve_storage_dir_for_read
from smarttutor.services.rag.pipelines.lightrag import config as lr_config
from smarttutor.services.rag.pipelines.lightrag import engine, storage
from smarttutor.services.rag.pipelines.lightrag.pipeline import LightRagPipeline
from smarttutor.services.rag.pipelines.lightrag.worker import run_in_worker_loop
...
def test_embedding_func_returns_numpy_array(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.embedding.get_embedding_config", lambda: _Config())
    monkeypatch.setattr("smarttutor.services.embedding.get_embedding_client", lambda: _Client())
...
def test_lightrag_llm_adapter_preserves_messages_and_drops_extra_kwargs(
    ...
    monkeypatch.setattr("smarttutor.services.llm.get_llm_client", lambda: _Client())
...
async def test_lightrag_vision_adapter_disables_provider_image_fallback(
    ...
    from smarttutor.services.llm.client import LLMClient
    from smarttutor.services.llm.config import LLMConfig
    from smarttutor.services.llm.multimodal import has_image_parts
    from smarttutor.services.llm.provider_core.base import LLMProvider, LLMResponse
    ...
    monkeypatch.setattr("smarttutor.services.llm.get_llm_client", lambda: client)
    monkeypatch.setattr("smarttutor.services.llm.factory.get_llm_config", lambda: config)
    monkeypatch.setattr(
        "smarttutor.services.llm.factory.get_runtime_provider",
...
def test_build_rag_skips_raganything_parser_install_check(monkeypatch) -> None:
    ...
    time, even though SmartTutor only ever inserts a pre-parsed ``content_list``
    ...
    assert captured["config"].working_dir.replace("\\", "/") == "/tmp/kb-wd"
...
def _stub_parse(monkeypatch, *, blocks=None, markdown: str = "# md") -> None:
    from smarttutor.services.parsing.types import ParsedDocument
    ...
    monkeypatch.setattr("smarttutor.services.parsing.get_parse_service", lambda: _Service())
...
def test_indexing_isolated_from_owner_loop_with_context_and_progress(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.parsing.get_parse_service", lambda: _ParseService())
