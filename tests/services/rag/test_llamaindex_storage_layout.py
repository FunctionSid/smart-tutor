...
from smarttutor.services.rag.index_versioning import EmbeddingSignature
...
async def test_incremental_add_migrates_matching_legacy_index_to_flat_version(
    ...
    from smarttutor.services.rag.pipelines.llamaindex import storage as storage_module
...
def test_hybrid_retriever_uses_official_query_fusion_when_bm25_available(
    ...
    from smarttutor.services.rag.pipelines.llamaindex import retrievers as retriever_module
    from smarttutor.services.rag.pipelines.llamaindex.config import RetrievalConfig
...
def test_retrieval_config_reads_profile_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    from smarttutor.services.rag.pipelines.llamaindex import config as config_module

    monkeypatch.setenv("SMARTTUTOR_RAG_RETRIEVAL_PROFILE", " vector ")
