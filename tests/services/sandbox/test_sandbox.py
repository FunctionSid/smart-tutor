...
from smarttutor.services.sandbox.backends import BwrapBackend, RestrictedSubprocessBackend
from smarttutor.services.sandbox.config import SandboxSettings, build_backend
from smarttutor.services.sandbox.quota import QuotaExceeded, UserExecQuota
from smarttutor.services.sandbox.service import SandboxService
from smarttutor.services.sandbox.spec import (
    ExecRequest,
    ExecResult,
    IsolationLevel,
    Mount,
...
def test_backend_selection_runner_url() -> None:
    from smarttutor.services.sandbox.backends import RunnerSidecarBackend
...
def test_backend_selection_none_without_optin() -> None:
    ...
    from smarttutor.services.sandbox.backends import RestrictedSubprocessBackend
...
def test_bwrap_binds_usr_local_when_available(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    if os.name == "nt":
        pytest.skip("bwrap is a Linux sandbox backend")
    usr = tmp_path / "usr"
...
def test_runner_server_validates_request_shape() -> None:
    from smarttutor.services.sandbox.runner import server
...
def test_runner_server_rejects_workdir_outside_allowed_roots(
    ...
    outside = server.execute({"command": "python -c \"pass\"", "workdir": str(tmp_path / "elsewhere")})
    ...
    via_link = server.execute({"command": "python -c \"pass\"", "workdir": str(sneaky)})
    assert "outside the shared workspace roots" in via_link["error"]

    inside = server.execute(
        {"command": "python -c \"pass\"", "workdir": str(allowed), "limits": {"timeout_s": 5}}
    )
    assert inside["error"] == ""
