...
from smarttutor.services.search.providers.brave import BraveProvider
from smarttutor.services.search.providers.jina import JinaProvider
from smarttutor.services.search.providers.serper import SerperProvider
from smarttutor.services.search.providers.tavily import TavilyProvider
...
def calls(monkeypatch):
    ...
    for module in ("serper", "tavily", "brave", "jina"):
        target = f"smarttutor.services.search.providers.{module}.requests"
...
def test_jina_truncates_results_to_max_results(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.providers.jina.requests", _FakeRequests)
...
def test_provider_metadata_comes_from_the_spec_table() -> None:
    from smarttutor.services.config import SEARCH_PROVIDERS
...
def _provider_class(module: str, name: str):
    ...
    return getattr(importlib.import_module(f"smarttutor.services.search.providers.{module}"), name)
...
def new_calls(monkeypatch):
    ...
    for module, _, _ in _NEW_PROVIDERS:
        ...
        monkeypatch.setattr(f"smarttutor.services.search.providers.{module}.requests", _FakeRequests)
    return captured
...
def test_doubao_reads_answer_and_citations_off_annotations(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.providers.doubao.requests", _FakeRequests)

    from smarttutor.services.search.providers.doubao import DoubaoProvider
...
def test_qianfan_maps_its_native_citation_fields(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.providers.qianfan.requests", _FakeRequests)

    from smarttutor.services.search.providers.qianfan import QianfanProvider
...
def test_aliyun_iqs_caps_results_client_side(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.providers.aliyun_iqs.requests", _FakeRequests)

    from smarttutor.services.search.providers.aliyun_iqs import AliyunIQSProvider
...
def test_bocha_surfaces_an_error_carried_inside_a_200(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.providers.bocha.requests", _FakeRequests)

    from smarttutor.services.search.providers.bocha import BochaProvider
