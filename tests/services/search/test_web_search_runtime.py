...
from smarttutor.services.config.provider_runtime import ResolvedSearchConfig
from smarttutor.services.search import web_search
from smarttutor.services.search.types import WebSearchResponse
...
def _patch_runtime(monkeypatch, resolved: ResolvedSearchConfig, **kwargs) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.search._get_web_search_config",
        lambda: {"enabled": True},
    )
    monkeypatch.setattr(
        "smarttutor.services.search.resolve_search_runtime_config",
        lambda: resolved,
    )
    monkeypatch.setattr(
        "smarttutor.services.search.search_fallback_candidates",
        lambda _provider: list(kwargs.get("candidates", [])),
    )
    monkeypatch.setattr(
        "smarttutor.services.search.search_provider_credentials",
...
def test_web_search_missing_key_falls_back_to_duckduckgo(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smarttutor.services.search.get_provider", _fake_get_provider)
...
def test_web_search_raises_when_every_candidate_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smarttutor.services.search.get_provider",
