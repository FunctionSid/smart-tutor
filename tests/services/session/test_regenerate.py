...
from smarttutor.services.session.turn_runtime import (
    TurnRuntimeManager,
...
class TestRegenerateLastTurn:
    ...
    async def test_end_to_end_skips_memory_refresh_and_no_duplicate_user(
        ...
        monkeypatch.setattr(
            "smarttutor.services.llm.config.get_llm_config", lambda: SimpleNamespace()
        )
        monkeypatch.setattr(
            "smarttutor.services.session.context_builder.ContextBuilder",
            FakeContextBuilder,
        )
