...
from smarttutor.services.session.pocketbase_store import PocketBaseSessionStore
from smarttutor.services.session.sqlite_store import SQLiteSessionStore
from smarttutor.services.session.turn_runtime import TurnRuntimeManager, _TurnExecution
...
def fake_pb(monkeypatch):
    client = _FakeClient()
    monkeypatch.setattr(
        "smarttutor.services.pocketbase_client.get_pb_client", lambda: client, raising=True
    )
    return client
...
def stub_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "smarttutor.services.session.turn_runtime.get_path_service",
