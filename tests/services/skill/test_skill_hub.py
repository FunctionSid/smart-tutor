...
def test_clawhub_catalog_normalises_rows() -> None:
    ...
    assert row["owner_url"] == "https://smarttutor.info"
...
def test_clawhub_detail_includes_body_version_and_tags() -> None:
    ...
    assert detail["owner"] == "SmartTutor"  # lifted from the envelope top level


def test_clawhub_web_origin_strips_api_suffix() -> None:
    provider = ClawHubProvider("eduhub", base_url="https://eduhub.smarttutor.info/api/v1")
    assert provider.web_origin == "https://eduhub.smarttutor.info"
