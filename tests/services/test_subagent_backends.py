...
from smarttutor.services.subagent import process as process_mod
from smarttutor.services.subagent.claude_code import ClaudeCodeBackend
from smarttutor.services.subagent.codex import CodexBackend
from smarttutor.services.subagent.config import (
    CONSULT_BUDGET_MAX,
...
from smarttutor.services.subagent.process import resolve_cli_command, stream_process_lines
from smarttutor.services.subagent.types import ConsultResult
...
def test_claude_command_applies_model_effort_system_prompt() -> None:
    backend = ClaudeCodeBackend()
    cfg = BackendConfig(model="opus", effort="high", system_prompt="consulted by SmartTutor")
    ...
    assert "--append-system-prompt" in cmd and "consulted by SmartTutor" in cmd
...
def test_materialize_images_writes_only_resolvable_images(tmp_path) -> None:
    ...
    from smarttutor.core.context import Attachment
    from smarttutor.services.subagent.images import materialize_images
...
def test_parse_claude_model_screen() -> None:
    from smarttutor.services.subagent.claude_models import _parse_model_screen
...
def test_claude_models_cache_roundtrip(monkeypatch, tmp_path) -> None:
    from smarttutor.services.subagent import claude_models as cm
...
async def test_claude_options_prefers_synced_cache(monkeypatch) -> None:
    ...
    curated fallback."""
    from smarttutor.services.subagent import claude_models as cm
    from smarttutor.services.subagent import models as models_mod
...
def test_session_registry_roundtrip(monkeypatch, tmp_path) -> None:
    from smarttutor.services.subagent import sessions as sess
...
def test_registry_partner_is_non_cli_backend() -> None:
    from smarttutor.services.subagent import PARTNER_BACKEND_KIND, get_backend, list_backend_kinds
...
async def test_detect_all_excludes_partner_backend() -> None:
    from smarttutor.services.subagent import detect_all
...
def _patch_manager(monkeypatch, manager) -> None:
    import smarttutor.services.partners as partners_pkg
...
async def test_partner_consult_mints_session_key_and_returns_reply(monkeypatch) -> None:
    ...
    from smarttutor.services.subagent.partner import PartnerBackend
...
def test_partner_event_mapping_covers_channels() -> None:
    from smarttutor.core.stream import StreamEvent, StreamEventType
    from smarttutor.services.subagent.partner import _to_subagent_events
...
def test_gemini_command_build_fresh_resume_and_approval_mapping() -> None:
    from smarttutor.services.subagent.gemini import GeminiBackend
...
def test_kimi_command_build_session_yolo_and_thinking() -> None:
    from smarttutor.services.subagent.kimi import KimiBackend
...
async def _drive_opencode(events, *, auto_approve=True):
    from smarttutor.services.subagent.opencode_family import OpencodeBackend
...
def test_opencode_final_text_skips_synthetic_parts() -> None:
    from smarttutor.services.subagent.opencode_family import _text_from_parts
...
def test_server_registry_reaps_idle_and_dead_handles() -> None:
    from smarttutor.services.subagent import opencode_server as srv
