...
from smarttutor.services import suggestions
from smarttutor.services.memory.recall import RecallHit
...
def isolated_scope(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    import smarttutor.services.path_service as path_service
...
def _stub_llm(monkeypatch: pytest.MonkeyPatch, reply: str) -> list[str]:
    ...
    import smarttutor.services.llm as llm
...
def test_l3_renders_sections_and_bullets(monkeypatch: pytest.MonkeyPatch) -> None:
    import smarttutor.services.memory as memory
