...
from smarttutor.services.voice.base import (
    VoiceProviderError,
    build_auth_headers,
    join_audio_path,
...
def test_resolve_tts_config_supports_global_edge_tts() -> None:
    catalog = {
        "version": 1,
        "services": {
            "tts": {
                ...
                "profiles": [
                    {
                        "id": "p",
                        "binding": "edge-tts",
                        "base_url": "C:/Python/Scripts/edge-tts.exe",
                        "api_key": "",
                        "models": [
                            {
                                "id": "m",
    ...
    assert cfg.adapter == "global_edge_tts"
    assert cfg.base_url == "C:/Python/Scripts/edge-tts.exe"
    assert cfg.api_key == "sk-no-key-required"


def test_resolve_stt_config_supports_global_faster_whisper() -> None:
    catalog = {
        "version": 1,
        "services": {
            "stt": {
                ...
                "profiles": [
                    {
                        ...
                        "base_url": "C:/Python/python.exe",
                        "api_key": "",
                        "models": [{"id": "m", "model": "base"}],
                    }
                ],
            }
        },
    ...
    assert cfg.provider_name == "faster_whisper"
...
@pytest.mark.asyncio
async def test_global_faster_whisper_adapter_invokes_configured_python(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[list[str]] = []
    ...
    async def fake_run(args: list[str], *, input_text=None, timeout: int) -> str:
        calls.append(args)
        return json.dumps({"text": "hello from local whisper"})

    monkeypatch.setattr(
        "smarttutor.services.voice.adapters.global_tools._run_subprocess",
        fake_run,
    )
    cfg = STTConfig(model="base", base_url="python", adapter="global_faster_whisper")
    text = await GlobalFasterWhisperSTTAdapter().transcribe(b"audio", cfg, filename="clip.wav")

    assert text == "hello from local whisper"
    assert calls[0][0].lower().endswith(("python.exe", "python"))
    assert calls[0][3] == "base"


@pytest.mark.asyncio
async def test_global_edge_tts_adapter_returns_mp3(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_run(args: list[str], *, input_text=None, timeout: int) -> str:
    ...
    cfg = TTSConfig(model="edge-tts", base_url="edge-tts", voice="en-US-AriaNeural")
    audio, content_type = await GlobalEdgeTTSTTSAdapter().synthesize("hello", cfg)
...
@pytest.mark.asyncio
async def test_global_piper_adapter_requires_model_path() -> None:
    cfg = TTSConfig(model="", base_url="piper", voice="")
    with pytest.raises(VoiceProviderError, match="no voice model path"):
        await GlobalPiperTTSAdapter().synthesize("hello", cfg)


@pytest.mark.asyncio
async def test_global_pyttsx3_adapter_returns_wav(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_run(args: list[str], *, input_text=None, timeout: int) -> str:
        output = Path(args[3])
    ...
    cfg = TTSConfig(model="pyttsx3", base_url="python", voice="Microsoft Zira Desktop")
    audio, content_type = await GlobalPyttsx3TTSAdapter().synthesize("hello", cfg)
...
@pytest.mark.asyncio
async def test_global_gtts_adapter_returns_mp3(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_run(args: list[str], *, input_text=None, timeout: int) -> str:
        output = Path(args[3])
    ...
    cfg = TTSConfig(model="en", base_url="python", voice="en")
    audio, content_type = await GlobalGTTSTTSAdapter().synthesize("hello", cfg)
