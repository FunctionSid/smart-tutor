...
/**
 * Revision dashboard — the persistent screen of the mastery experience.
 *
 * The tutoring itself runs on the chat agent loop (pick Revision mode in
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-[var(--border)]">
        <header className="border-b border-[var(--border)] px-4 py-3">
          ...
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">
            {tr(
              "掌握式学习：硬门槛 + 间隔复习",
              "Spaced review with mastery gates",
            )}
          </p>
        </header>
        <div className="flex-1 space-y-1 overflow-y-auto p-2">
          {loadingList ? (
            ...
            <p className="px-2 py-3 text-xs leading-relaxed text-[var(--muted-foreground)]">
              {tr(
                "还没有复习路径。去「导师」让 Smart Tutor 根据你的材料建一条。",
                "No revision paths yet. Open Tutor and ask Smart Tutor to build one from your materials.",
              )}
            </p>
          ) : (
            paths.map((path) => (
        ...
        {loadingDetail && !detail ? (
          ...
          <div className="flex h-full flex-col items-center justify-center px-6 text-center text-[var(--muted-foreground)]">
            ...
            <p className="max-w-sm text-sm leading-relaxed">
              {tr(
                "选择一条复习路径查看进度地图，或在「导师」里开始。",
                "Select a revision path to see its progress map, or start one in Tutor.",
              )}
            </p>
          </div>
        ) : (
