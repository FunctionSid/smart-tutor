...
export default function ChatPage() {
  ...
  // A message handed over by another page (Settings' "set up with Smart Tutor"
  ...
  useEffect(() => {
    ...
    if (intent.knowledgeBases.length) {
      setKBs(Array.from(new Set(intent.knowledgeBases)));
    }
    ...
    if (intent.readerMaterialId) {
      void openMaterial({
        material_id: intent.readerMaterialId,
        filename: "",
        unit: "page",
        unit_count: 0,
        mime: "",
  ...
  // How many times Smart Tutor may consult the selected agent this turn. Seeded
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      // (how many times Smart Tutor may ask it) so the subagent capability uses it.
    ...
    () => [
      {
        label: "Teach me",
        prompt:
          "Teach me the selected material step by step. Use the open Reader document and selected Library sources as the grounding, check my understanding as we go, and cite the document when you use it.",
      },
      {
        label: "Explain",
        prompt:
          "Explain the current passage or main idea from the selected material. Ground the explanation in the open document or selected Library source and cite the evidence you use.",
      },
      {
        label: "Simplify",
        prompt:
          "Simplify the selected material for a beginner. Keep the answer grounded in the document, define any jargon, and include a tiny example.",
      },
      {
        label: "Summarize",
        prompt:
          "Summarize the selected material. Separate the core ideas, important details, and anything I should review again, using citations from the document where available.",
      },
      {
        label: "Quiz",
        prompt:
          "Create a short quiz from the selected material, then wait for my answers before grading. Focus on the most important ideas and cite the source passages after each explanation.",
      },
      {
        label: "Test",
        prompt:
          "Test my understanding of the selected material with a mix of conceptual and application questions. Ask one question at a time and track what I miss.",
      },
      {
        label: "Practice",
        prompt:
          "Give me targeted practice on the selected material. Start with one worked example, then give me a similar problem to try, and adapt based on my answer.",
      },
      {
        label: "Revise",
        prompt:
          "Help me revise this material. Build a focused review plan from the selected sources, prioritize weak topics, and start with the first review prompt.",
      },
      {
        label: "Weak topics",
        prompt:
          "Identify my weak topics from this session, my saved progress, and the selected material. Explain the evidence for each weak topic and recommend what to practice next.",
      },
    ],
    [],
  );
...
function LearningActionStrip({
  ...
  const { t } = useTranslation();
  return (
    <div className="mx-auto w-full max-w-[960px] px-6 pb-2">
      <div
        className="flex gap-1.5 overflow-x-auto py-1"
        aria-label={t("Learning actions")}
      >
        {actions.map((action) => (
          <button
            key={action.label}
            ...
            className="inline-flex h-7 shrink-0 items-center rounded-lg border border-[var(--border)] px-2.5 text-[11.5px] font-medium text-[var(--muted-foreground)] transition-colors hover:border-[var(--ring)] hover:text-[var(--foreground)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40 disabled:cursor-not-allowed disabled:opacity-45"
          >
            {t(action.label)}
          </button>
        ))}
...
function SubagentTabWatcher({
  ...
  useEffect(() => {
    // Group by turn so all of one turn's consults (Smart Tutor may ask the agent
