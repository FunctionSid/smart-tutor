...
/** "Smart Tutor_Introduction.pdf" → "Smart Tutor Introduction" — the card title
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("Smart Tutor Exploring… · 8s" → "Smart Tutor responded. · 10s") with
          the exploring trace nested beneath it — expanded while Smart Tutor is
  ...
  const play = useCallback(async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        setError(detail?.detail || t("Could not play this reply aloud."));
        return;
      }
      ...
      audio.onerror = () => {
        setState("idle");
        setError(t("Audio playback failed."));
        cleanup();
      };
      await audio.play();
      setState("playing");
      ...
      cleanup();
      setState("idle");
      setError(t("Could not play this reply aloud."));
    }
  }, [cleanup, content, t]);
