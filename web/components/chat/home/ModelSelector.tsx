...
function aiProviderLabel(option: LLMOption | null) {
  if (!option) return "AI";
  const provider = providerLabel(option);
  return option.provider_category === "local"
    ? `AI: Local · ${provider}`
    : `AI: ${provider}`;
}
...
export default function ModelSelector({
  ...
  const groupedOptions = useMemo(() => {
    ...
    for (const option of options) {
      const key =
        option.provider_category === "local"
          ? `Local · ${providerLabel(option)}`
          : providerLabel(option);
  ...
  const indicatorText =
    selectedOption && !loading && !error
      ? `${aiProviderLabel(selectedOption)} · ${selectedOption.model || selectedOption.model_name}`
      : loading
        ? t("AI model loading")
        : error
          ? t("AI models unavailable")
          : t("AI model not selected");
  ...
  return (
    <div ref={rootRef} className="relative flex min-w-0 items-center gap-1.5">
      <div
        aria-live="polite"
        className="max-w-[220px] truncate text-[11px] font-medium text-[var(--muted-foreground)]"
        title={indicatorText}
      >
        {indicatorText}
      </div>
      ...
      <button
        ...
        onClick={() => {
          ...
          setOpen((current) => {
            ...
            if (
              next &&
              selectedOption?.provider_category === "local" &&
              onRefresh
            ) {
              onRefresh();
            }
        ...
        aria-label={
          canRefresh
            ? t("Refresh models")
            : t("Select AI model. Current selection: {{selection}}", {
                selection: indicatorText,
              })
        }
        ...
        >
          ...
          <div className="max-h-[280px] overflow-y-auto py-1">
            ...
            {groupedOptions.map(([groupLabel, groupOptions]) => (
              <div key={groupLabel}>
                <div className="px-3 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-wide text-[var(--muted-foreground)]/70">
                  {groupLabel}
                </div>
