...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "Smart Tutor Reflecting…"
 ...
 *  outside Smart Tutor that the turn is talking to. */
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "Smart Tutor Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "Smart Tutor";
  ...
  else if (mode === "responded") modeLabel = t("Smart Tutor responded.");
 ...
 * reasoning trace once Smart Tutor stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("Smart Tutor Exploring… · 8s", settling to "Smart Tutor responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while Smart Tutor is still reasoning /
