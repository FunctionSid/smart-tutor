...
export default function KbDocumentsSection({
  ...
  const currentProgressMessage = kb.progress
    ? progressMessage(kb.progress, t)
    : undefined;
  const duplicateCount = kb.progress?.duplicate_count ?? 0;
  const showTaskLogs =
    task?.kind === "upload" ||
    task?.kind === "create" ||
    task?.kind === "reindex" ||
        ...
        (task.taskId || task.logs.length > 0 || task.executing) && (
          <div className="space-y-2">
            <div
              ...
              aria-live="polite"
              aria-atomic="true"
              className="space-y-1 text-[11px] text-[var(--muted-foreground)]"
            >
