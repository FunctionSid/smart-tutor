...
import { newReaderChatUrl } from "@/lib/chat-launch-intent";
...
interface KbFilePreviewProps {
  ...
  /** Source KB to attach as RAG scope when opening this file in Tutor. */
...
export default function KbFilePreview({
  ...
  const [openReaderBusy, setOpenReaderBusy] = useState(false);
  const [openReaderError, setOpenReaderError] = useState<string | null>(null);
  ...
  const handleOpenInReader = async () => {
    if (!previewUrl || openReaderBusy) return;
    setOpenReaderBusy(true);
    setOpenReaderError(null);
    try {
      const response = await apiFetch(previewUrl);
      if (!response.ok) {
        throw new Error(`Could not read ${source.filename} (${response.status}).`);
      }
      ...
      setOpenReaderError(
        error instanceof Error
          ? error.message
          : t("This file could not be opened in the Reader."),
      );
    } finally {
      setOpenReaderBusy(false);
    }
  };
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--card)]/80 px-3 py-2">
        ...
        {previewUrl && (
          <button
            type="button"
            onClick={() => void handleOpenInReader()}
            disabled={openReaderBusy}
            title={t("Open in Reader")}
            aria-label={t("Open in Reader")}
            className="flex h-7 w-7 items-center justify-center rounded-md text-[var(--muted-foreground)] transition-colors hover:bg-[var(--muted)] hover:text-[var(--foreground)] disabled:opacity-50"
          >
            {openReaderBusy ? (
      ...
      {openReaderError && (
        <div
          role="alert"
          className="border-b border-[var(--destructive)]/25 bg-[var(--destructive)]/[0.06] px-3 py-2 text-[11.5px] text-[var(--destructive)]"
        >
          {openReaderError}
        </div>
      )}
