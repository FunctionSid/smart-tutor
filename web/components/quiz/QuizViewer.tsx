...
export default function QuizViewer({
  ...
  const answerFieldId = `quiz-${questionKey}-answer`;
  const choiceGroupName = `quiz-${questionKey}-choice`;
        ...
        {isChoice ? (
          <fieldset className="space-y-1.5">
            <legend className="sr-only">
              {t("Answer choices for question {{number}}", { number: idx + 1 })}
            </legend>
            {Object.entries(q.options!).map(([key, text]) => {
              ...
              return (
                <label
                  key={key}
                  className={`flex w-full cursor-pointer items-start gap-2.5 rounded-lg border px-3 py-2 text-left text-[13px] transition-all has-[:disabled]:cursor-not-allowed ${optionClass}`}
                >
                  <input
                    type="radio"
                    ...
                    onChange={() => updateAnswer({ selected: key })}
                    className="sr-only"
                  />
                  <span
          ...
          // Concept (true/false) — two large radio choices.
          (() => {
            const correctTF = resolveConceptAnswer(q.correct_answer);
            const showFeedback = ans.submitted;
            const renderTFChoice = (key: "true" | "false", label: string) => {
              const isSelected = ans.selected === key;
              ...
              return (
                <label
                  key={key}
                  className={`flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3 py-3 text-[14px] font-semibold transition-all has-[:disabled]:cursor-not-allowed ${cls}`}
                >
                  <input
                    type="radio"
                    name={choiceGroupName}
            ...
            return (
              <fieldset className="flex gap-2">
                <legend className="sr-only">
                  {t("True or false answer for question {{number}}", {
                    number: idx + 1,
                  })}
                </legend>
                {renderTFChoice("true", t("True"))}
                {renderTFChoice("false", t("False"))}
              </fieldset>
            );
          })()
        ) : isFillBlank ? (
          ...
          <div>
            <label
              ...
              className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]/70"
            >
              {t("Fill in the Blank")}
            </label>
            <input
          ...
          <div>
            <label htmlFor={answerFieldId} className="sr-only">
              {t("Answer for question {{number}}", { number: idx + 1 })}
            </label>
            <textarea
              id={answerFieldId}
              value={ans.typed}
