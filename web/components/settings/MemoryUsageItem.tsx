 ...
 *
 * Backed by `/api/v1/system/memory` (smarttutor/runtime/memory_probe.py), which
 * walks the whole Smart Tutor process tree — backend, the Next.js server, and
    ...
    <Fragment>
      ...
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1.5 text-[12px] leading-none text-[var(--muted-foreground)]">
        ...
        {usage.partial && (
          <span
            title={t(
              "Only the backend and its own children. Start Smart Tutor through the launcher to include the web server.",
            )}
          >
            {t("(backend only)")}
          </span>
