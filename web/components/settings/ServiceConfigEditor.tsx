...
export function ServiceConfigEditor({ service }: { service: ServiceName }) {
  ...
  const selectTutorProvider = (binding: "openai" | "ollama") => {
    ...
    mutateCatalog((next) => {
      ...
      const existing = target.profiles.find(
        (profile) => profile.binding === binding,
      );
      if (existing) {
        target.active_profile_id = existing.id;
        target.active_model_id = existing.models[0]?.id ?? null;
        return;
      }

      const providerOption = (providers.llm || []).find(
      ...
      target.profiles.push({
        ...
        extra_headers: {},
        models: modelName
          ? [{ id: modelId, name: modelName, model: modelName }]
          : [],
      });
      target.active_profile_id = profileId;
      target.active_model_id = modelName ? modelId : null;
    });
    setToast(
      binding === "ollama"
            ...
            {[
              ...
              return (
                <button
                  ...
                  onClick={() => selectTutorProvider(item.value)}
                  className={`rounded-lg border px-3 py-2 text-left transition-colors ${
                    selected
                      ? "border-[var(--primary)] bg-[var(--primary)]/[0.06]"
                      : "border-[var(--border)] hover:bg-[var(--muted)]/35"
                  }`}
                >
...
function ProfileFields({
  ...
  const isLocalVoiceProvider =
    (service === "tts" || service === "stt") && providerOption?.is_local;
  ...
  const fields =
    isCodexOAuth || isCodeBuddyAuth
      ...
      : isLocalVoiceProvider
        ? { apiKey: false, baseUrl: true, baseUrlRequired: true }
      : service === "llm" && providerValue === "ollama"
        ? { apiKey: false, baseUrl: true, baseUrlRequired: true }
      : service === "search"
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      {fields.baseUrl && (
        <div className="sm:col-span-2">
          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
            {service === "embedding"
              ? t("Endpoint URL")
              : isLocalVoiceProvider && service === "stt"
                ? t("Global Python executable")
                : isLocalVoiceProvider && service === "tts"
                  ? t("Global executable or Python path")
              : service === "llm" && providerValue === "ollama"
                ? t("Ollama server URL")
                : t("Base URL")}
          </div>
          <input
            ...
            placeholder={
              service === "embedding"
                ...
                : isLocalVoiceProvider && service === "stt"
                  ? "C:\\Users\\Sourabh\\AppData\\Local\\Programs\\Python\\Python312\\python.exe"
                  : isLocalVoiceProvider && service === "tts"
                    ? "edge-tts"
                : service === "llm" && providerValue === "ollama"
                  ? "http://localhost:11434/v1"
          ...
          {service === "embedding" && (
            <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
              {t(
                "Embedding requests are sent to this URL exactly; Smart Tutor does not append /embeddings or /api/embed at request time.",
              )}
            </p>
          )}
          {isLocalVoiceProvider && (
            <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
              {service === "stt"
                ? t(
                    "Smart Tutor runs this global Python in a subprocess and imports the selected speech package there.",
                  )
                : t(
                    "Smart Tutor runs this global voice tool in a subprocess. Use an executable name on PATH or a full path.",
                  )}
            </p>
          )}
      ...
      {fields.apiKey && (
        <div className="sm:col-span-2">
          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
            {service === "llm" && providerValue === "openai"
              ? t("OpenAI API key")
              : isLocalVoiceProvider
                ? t("API key not required")
              : t("API Key")}
          </div>
          <div className="relative">
            <input
