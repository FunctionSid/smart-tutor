...
// Defaults mirror BackendConfig in smarttutor/services/subagent/config.py so the
  ...
  return (
    <div>
      <SettingsPageHeader
        title={displayName}
        description={tr({
          zh: `Smart Tutor 通过 consult_subagent 调用本机 ${displayName} 时使用的模型、推理强度与运行参数。设置后即覆盖 CLI 的默认值；留空表示沿用 CLI 默认。`,
          en: `Model, reasoning effort, and run parameters Smart Tutor drives the local ${displayName} with when consulting it. These override the CLI defaults; leave blank to keep the CLI's own default.`,
        })}
      />
      ...
      {!loading && options && (
        <>
          ...
          <SettingSection
            ...
            description={tr({
              zh: "Smart Tutor 调用该智能体时使用的模型与推理强度。",
              en: "The model and reasoning effort Smart Tutor consults this agent with.",
            })}
          >
            <SettingRow
              title={tr({ zh: "启用", en: "Enabled" })}
              description={tr({
                zh: "关闭后，Smart Tutor 不会在对话中调用该智能体。",
                en: "When off, Smart Tutor won't consult this agent in chat.",
              })}
              control={
                <Toggle
          ...
          {features.systemPrompt && (
            <SettingSection
              ...
              description={`${tr(
                ...
                zh: "留空则使用 Smart Tutor 的默认委派提示。",
                en: "Blank uses Smart Tutor's default delegate instruction.",
              })}`}
            >
              <div className="py-4">
                <textarea
          ...
          <SettingSection
            ...
            description={tr({
              zh: "Smart Tutor 无人值守地驱动该智能体——默认值确保它不会卡在等待确认上。",
              en: "Smart Tutor drives the agent unattended — the defaults ensure it never stalls waiting for an approval prompt.",
            })}
          >
            ...
            {features.forwardImages && (
              <SettingRow
                ...
                description={tr({
                  zh: "允许 Smart Tutor 把本轮对话中的图片附件转发给该智能体。",
                  en: "Let Smart Tutor forward image attachments from the chat turn to this agent.",
                })}
                control={
                  <Toggle
