...
export const resolveKnowledgeIndexFailure = (
  ...
  const requiresEmbeddingChange =
    embeddingConfigurationCodes.has(code ?? "") ||
    normalizedMessage.includes("no active embedding model") ||
    normalizedMessage.includes("no embedding model is configured");
  const requiresCompletionChange =
    completionConfigurationCodes.has(code ?? "") ||
    normalizedMessage.includes("no active llm model") ||
    normalizedMessage.includes("no active chat model");
