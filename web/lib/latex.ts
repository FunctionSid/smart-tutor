...
export function convertSequenceFenceToMermaid(source: string): string | null {
  ...
  const participantRef = (label: string): string => {
    const trimmed = label.trim();
    if (/^[A-Za-z][\w.-]*$/.test(trimmed)) return trimmed;

    const existing = participantAliases.get(trimmed);
    ...
    const base =
      trimmed
        ...
        .replace(/^_+|_+$/g, "")
        .replace(/^[^A-Za-z_]+/, "") || "Participant";
    let alias = base;
    let suffix = 2;
    while (aliasValues.has(alias)) {
      alias = `${base}_${suffix}`;
      suffix += 1;
    }
    participantAliases.set(trimmed, alias);
  ...
  const converted = normalizedLines.map((line) => {
    const noteMatch = /^Note\s+(left|right)\s+of\s+(.+?):\s*(.*)$/i.exec(line);
    if (noteMatch) {
      const [, side, label, note] = noteMatch;
      return `  Note ${side.toLowerCase()} of ${participantRef(label)}: ${note.replace(/\\n/g, "<br/>")}`;
    }
      ...
      const arrow =
        operator === "--" || operator === "-->"
          ...
          : operator === "->>" || operator === "-->>"
            ? operator
            : "->>";
      return `  ${participantRef(from)}${arrow}${participantRef(to)}: ${message}`;
    }

    return `  ${line}`;
  });

  const participants = [...participantAliases.entries()].map(
    ([label, alias]) => `  participant ${alias} as ${label}`,
  );

  return ["sequenceDiagram", ...participants, ...converted].join("\n");
}
