...
// truth at ``smarttutor/__version__.py``. The Python file is parsed with a
// small regex so the JS build does not need to execute Python.
const APP_VERSION = (() => {
  try {
    const text = fs.readFileSync(
      path.resolve(__dirname, "..", "smarttutor", "__version__.py"),
      "utf8",
    );
...
const nextConfig = {
  // Keep the production build used by `smarttutor start` separate from the
  // `.next` development cache used by the explicit `smarttutor start --dev`.
  // Without separate directories either command can invalidate the other
  // process while it is running.
  distDir: process.env.SMARTTUTOR_NEXT_DIST_DIR || ".next",
