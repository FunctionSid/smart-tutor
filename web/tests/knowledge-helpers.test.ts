...
test("resolveKnowledgeIndexFailure routes unstructured embedding setup errors", () => {
  const missingEmbedding = resolveKnowledgeIndexFailure(
    kb({
      status: "error",
      progress: {
        stage: "error",
  ...
  assert.equal(missingEmbedding?.requiresModelChange, true);
  assert.equal(missingEmbedding?.settingsHref, "/settings/embedding");
});
